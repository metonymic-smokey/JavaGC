...
class Buffer {
	...
	public Buffer(String fileName) {
		try {
			...
			throw new Parser.FatalError("Could not open file " + fileName);
		}
	}
	...
	protected void Close() {
		if (file != null) {
			try {
				...
				throw new Parser.FatalError(e.getMessage());
			}
		}
	}
	...
	public void setPos(int value) {
		...
		if (value < 0 || value > fileLen) {
			throw new Parser.FatalError("buffer out of bounds access, position: " + value);
		}
	...
	private int ReadNextStreamChunk() {
		...
		catch (IOException ioex) { throw new Parser.FatalError(ioex.getMessage()); }
...
public class Scanner {
	...
	static {
		...
		for (int i = 36; i <= 36; ++i) start.set(i, 2);
		for (int i = 65; i <= 90; ++i) start.set(i, 2);
		for (int i = 95; i <= 95; ++i) start.set(i, 2);
		for (int i = 97; i <= 122; ++i) start.set(i, 2);
		for (int i = 37; i <= 37; ++i) start.set(i, 1);
		...
		literals.put("package", new Integer(3));
		literals.put("namespace", new Integer(4));
		literals.put("Package", new Integer(5));
		literals.put("Namespace", new Integer(6));
		literals.put("ns", new Integer(7));
		literals.put("NS", new Integer(8));
		literals.put("DS", new Integer(11));
		literals.put("ds", new Integer(12));
		literals.put("Datastructure", new Integer(13));
		literals.put("DataStructure", new Integer(14));
		literals.put("datastructure", new Integer(15));
		literals.put("head", new Integer(16));
		literals.put("Head", new Integer(17));
	...
	void Init () {
		...
		if (ch == 0xEF) { // check optional byte order mark for UTF-8
			...
			if (ch1 != 0xBB || ch2 != 0xBF) {
				throw new Parser.FatalError("Illegal byte order mark at start of file");
			}
		...
		loop: for (;;) {
			switch (state) {
				...
				case 1:
					...
					if (ch >= '$' && ch <= '%' || ch == '*' || ch >= '0' && ch <= '9' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'z') {AddCh(); state = 1; break;}
					else {t.kind = 2; break loop;}
				case 2:
					recEnd = pos; recKind = 1;
					if (ch == '$' || ch >= '0' && ch <= '9' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'z') {AddCh(); state = 2; break;}
					else if (ch == '%' || ch == '*') {AddCh(); state = 3; break;}
					else {t.kind = 1; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
				case 3:
					recEnd = pos; recKind = 2;
					if (ch >= '$' && ch <= '%' || ch == '*' || ch >= '0' && ch <= '9' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'z') {AddCh(); state = 3; break;}
					else {t.kind = 2; break loop;}
				case 4:
					{t.kind = 9; break loop;}
				case 5:
