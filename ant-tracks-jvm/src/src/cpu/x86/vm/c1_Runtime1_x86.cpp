...
int StubAssembler::call_RT(Register oop_result1, Register metadata_result, address entry, int args_size) {
...
#ifdef _LP64
#ifdef _WIN64 //win64 does not support as many argument registers => put last arguments on the stack (cannot use align_stack, destroys arguments ...)
  {
      int rt_arguments = MAX2(args_size - 3, 0);
      if(rt_arguments > 0 && rt_arguments % 2 == 1) {
          assert(rt_arguments == 1, "no other case supported yet");
          pop(c_rarg0);
          push(c_rarg0);
          push(c_rarg0);
          rt_arguments++;
      }
      set_num_rt_args(rt_arguments);
  }
#else
  ...
  if(num_rt_args() > 0) {
      addptr(rsp, num_rt_args()*BytesPerWord);
  }
...
int StubAssembler::call_RT(Register oop_result1, Register metadata_result, address entry, Register arg1, Register arg2, Register arg3, Register arg4) {
#ifdef _LP64
  // if there is any conflict use the stack
    push(arg4);
    push(arg3);
...
int StubAssembler::call_RT(Register oop_result1, Register metadata_result, address entry, Register arg1, Register arg2, Register arg3, Register arg4, Register arg5) {
#ifdef _LP64
  // if there is any conflict use the stack
    push(arg5);
    push(arg4);
  ...
  push(arg1);
#endif // _LP64
  return call_RT(oop_result1, metadata_result, entry, 5);
}
  ...
  switch (id) {
    ...
    case fast_new_instance_init_check_id:
      {
        ...
        if ((id == fast_new_instance_id || id == fast_new_instance_init_check_id) &&
            UseTLAB && FastTLABRefill && (!TraceObjectsAllocations || TraceObjectsC1ExperimentalEnableFastTLABRefill)) {
          Label slow_path;
          ...
          if(TraceObjectsAllocations) {
              __ enter();
              Register allocation_site = rbx;
              OopMap* map = save_live_registers(sasm, 3);
              __ movptr(allocation_site, Address(rbp, (0 + 2) * BytesPerWord)); //StubFrame::load_argument(allocation_site, 0)
              int call_offset = __ call_RT(obj, noreg, CAST_FROM_FN_PTR(address, TraceObjectsUseNakedSlowPaths ? EventsC1Runtime::fire_obj_alloc_fast_with_oop_naked : EventsC1Runtime::fire_obj_alloc_fast_with_oop), obj, allocation_site);
              if(oop_maps == NULL) oop_maps = new OopMapSet();
          ...
          if(TraceObjectsAllocations) {
              ...
              OopMap* map = save_live_registers(sasm, 3);
              __ movptr(allocation_site, Address(rbp, (0 + 2) * BytesPerWord)); //StubFrame::load_argument(allocation_site, 0)
              int call_offset = __ call_RT(obj, noreg, CAST_FROM_FN_PTR(address, TraceObjectsUseNakedSlowPaths ? EventsC1Runtime::fire_obj_alloc_normal_with_oop_naked : EventsC1Runtime::fire_obj_alloc_normal_with_oop), obj, allocation_site);
              if(oop_maps == NULL) oop_maps = new OopMapSet();
              oop_maps->add_gc_map(call_offset, map);
              restore_live_registers_except_rax(sasm);
              __ verify_oop(obj);
        ...
        StubFrame f(sasm, "new_instance_id", dont_gc_arguments);
        OopMap* map = save_live_registers(sasm, 3);
        Register allocation_site = rbx;
        if(TraceObjectsAllocations){
            f.load_argument(0, allocation_site);
        }

        int call_offset = __ call_RT(obj, noreg, CAST_FROM_FN_PTR(address, new_instance), klass, allocation_site);
        if(oop_maps == NULL) oop_maps = new OopMapSet();
        oop_maps->add_gc_map(call_offset, map);
        restore_live_registers_except_rax(sasm);
        __ verify_oop(obj);
    ...
    case new_object_array_id:
      {
        ...
        if (UseTLAB && FastTLABRefill && (!TraceObjectsAllocations || TraceObjectsC1ExperimentalEnableFastTLABRefill)) {
        ...
        StubFrame f(sasm, "new_array_id", dont_gc_arguments);
        OopMap* map = save_live_registers(sasm, 3);
        if(TraceObjectsAllocations){
            f.load_argument(0, rdi); // rdi: rcx,: AllocationSite
        }

        int call_offset;
        if (id == new_type_array_id) {
          call_offset = __ call_RT(obj, noreg, CAST_FROM_FN_PTR(address, new_type_array), klass, length, rdi);
        } else {
          call_offset = __ call_RT(obj, noreg, CAST_FROM_FN_PTR(address, new_object_array), klass, length, rdi);
        }
    ...
    case new_multi_array_id:
      { StubFrame f(sasm, "new_multi_array", dont_gc_arguments);
        ...
        __ andptr(rbx, 0xFFFFFFFF);
        __ shlptr(rsi, (HeapWordSize - SIZE_OF_ALLOCATION_SITE_IDENTIFIER_SMALL) * 8);
        __ orptr(rbx, rsi);
    ...
    case dtrace_object_alloc_id:
      ...
      break;
    case fire_obj_alloc_fast_with_event_id:
      {
        Register obj = rax; // Incoming and outgoing
        Register event = rdi; // Incoming
        StubFrame f(sasm, "fire_obj_alloc_fast_with_event", dont_gc_arguments);
        OopMap* map = save_live_registers(sasm, 3);

        int call_offset = __ call_RT(obj, noreg, CAST_FROM_FN_PTR(address, EventsC1Runtime::fire_obj_alloc_fast_with_event_naked), obj, event);
      ...
      break;
    case fire_obj_alloc_fast_with_oop_id:
      {
        Register obj = rax; // Incoming and outgoing
        Register allocation_site = rbx; //temp register for argument
        StubFrame f(sasm, "fire_obj_alloc_fast_with_oop", dont_gc_arguments);
        OopMap* map = save_live_registers(sasm, 3);
      ...
      break;
    case store_alloc_site_inlined_id:
      {
          Register obj = rax; // incoming and outgoing
          Register allocation_site = rdi; // incoming
          if(!TraceObjectsC1SaveAllocationSitesAlwaysSlowPath) {
              Register t1 = rbx, t2 = rcx, t3 = rdx;
              __ push(t1); __ push(t2); __ push(t3);
              Label slow_path, end;
              bool needs_slow_path = __ emit_store_allocation_site_inlined(obj, t1, t2, t3, RegisterOrConstant(allocation_site), &slow_path, &end, false, true, NULL);
              //__ verify_oop(obj);
              __ bind(end);
              __ pop(t3); __ pop(t2); __ pop(t1);
              __ ret(0);
              if(needs_slow_path) {
                  __ bind(slow_path);
                  __ pop(t3); __ pop(t2); __ pop(t1);
    ...
    case store_alloc_site_id:
      {
        Register obj = rax; // incoming and outgoing
        Register allocation_site = rdi; // incoming
        StubFrame f(sasm, "store_alloc_site", dont_gc_arguments);
        OopMap* map = save_live_registers(sasm, 3);
        
        //this must be a naked call, otherwise the there might be a gc before the event is fired
        int call_offset = __ call_RT(obj, noreg, CAST_FROM_FN_PTR(address, EventsC1Runtime::store_alloc_site_naked), obj, allocation_site);
