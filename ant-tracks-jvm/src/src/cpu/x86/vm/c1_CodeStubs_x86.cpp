...
void NewInstanceStub::emit_code(LIR_Assembler* ce) {
  ...
  if(TraceObjectsAllocations){
        C1CallSiteIterator call_sites = C1CallSiteIterator(_info->stack());
        ce->store_parameter(AllocationSites::method_to_allocation_site(&call_sites, (Klass*) _klass->constant_encoding()), 0);
  } else {
        ce->store_parameter(0, 0);
  }
...
FireAllocFastStub::FireAllocFastStub(CodeEmitInfo* info, Klass* klass) {
    _info = new CodeEmitInfo(info->stack(), NULL, false);
    _klass = klass;
}

void FireAllocFastStub::emit_code(LIR_Assembler* ce) {
  assert(__ rsp_offset() == 0, "frame size should be fixed");
  __ bind(_entry);
  
  
  C1CallSiteIterator call_sites = C1CallSiteIterator(_info->stack());
  ce->store_parameter(AllocationSites::method_to_allocation_site(&call_sites, _klass),  0);
  
  __ call(RuntimeAddress(Runtime1::entry_for(Runtime1::fire_obj_alloc_fast_with_oop_id)));
...
NewTypeArrayStub::NewTypeArrayStub(LIR_Opr klass_reg, LIR_Opr length, LIR_Opr result, CodeEmitInfo* info, Klass* klass) {
  _klass_reg = klass_reg;
  _length = length;
  _result = result;
  _info = new CodeEmitInfo(info);
...
NewObjectArrayStub::NewObjectArrayStub(LIR_Opr klass_reg, LIR_Opr length, LIR_Opr result, CodeEmitInfo* info, Klass* klass) {
  _klass_reg = klass_reg;
  _result = result;
  _length = length;
  _info = new CodeEmitInfo(info);
