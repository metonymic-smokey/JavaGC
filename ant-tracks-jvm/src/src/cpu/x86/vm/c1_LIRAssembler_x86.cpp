...
void LIR_Assembler::emit_alloc_obj(LIR_OpAllocObj* op) {
  ...
  C1CallSiteIterator call_sites = C1CallSiteIterator(op->stub()->info()->stack());
  __ allocate_object(op->obj()->as_register(),
                     ...
                     *op->stub()->entry(),
                     op->trace_stub() != NULL ? op->trace_stub()->entry() : NULL,
                     &call_sites,
                     op->allocated_type());
  __ bind(*op->stub()->continuation());
  if(op->trace_stub() != NULL) __ bind(*op->trace_stub()->continuation());
}
...
void LIR_Assembler::emit_alloc_array(LIR_OpAllocArray* op) {
  ...
  if (UseSlowPath || op->allocated_type() == NULL ||
