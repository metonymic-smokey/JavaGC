...
#include "prims/EventsC1CallSites.hpp"
...
void LIRGenerator::do_NewTypeArray(NewTypeArray* x) {
  ...
  Metadata* ci_type_arr_klass = ciTypeArrayKlass::make(elem_type)->constant_encoding();
  __ metadata2reg(ci_type_arr_klass, klass_reg);

  Klass* allocated_type = (Klass*) (ci_type_arr_klass);
  CodeStub* slow_path = new NewTypeArrayStub(klass_reg, len, reg, info, allocated_type);
  CodeStub* trace_slow_path = TraceObjectsAllocations && TraceObjectsC1RelocateSlowPath ? new FireAllocFastStub(info, allocated_type) : NULL;
  __ allocate_array(reg, len, tmp1, tmp2, tmp3, tmp4, elem_type, klass_reg, slow_path, trace_slow_path, allocated_type);
...
void LIRGenerator::do_NewObjectArray(NewObjectArray* x) {
  ...
  Klass* allocated_type = (Klass*) obj->constant_encoding();
  //allocated_type may be null if not yet loaded
  
  CodeStub* slow_path = new NewObjectArrayStub(klass_reg, len, reg, info, allocated_type);
  CodeStub* trace_slow_path = TraceObjectsAllocations && TraceObjectsC1RelocateSlowPath ? new FireAllocFastStub(info, allocated_type) : NULL;
  ...
  __ allocate_array(reg, len, tmp1, tmp2, tmp3, tmp4, T_OBJECT, klass_reg, slow_path, trace_slow_path, allocated_type);
...
void LIRGenerator::do_NewMultiArray(NewMultiArray* x) {
  ...
  LIR_Opr allocation_site = FrameMap::rsi_opr;
  
  if(TraceObjectsAllocations){
      C1CallSiteIterator call_sites = C1CallSiteIterator(x->state());
      AllocationSiteIdentifier allocation_site_id = AllocationSites::method_to_allocation_site(&call_sites, (Klass*) (x->klass()->constant_encoding()));
      __ move(LIR_OprFact::intConst(allocation_site_id), allocation_site);
  } else {
      __ move(LIR_OprFact::intConst(0), allocation_site);
  }
  ...
  LIR_OprList* args = new LIR_OprList(5);
  args->append(klass_reg);
  args->append(rank);
  args->append(varargs);
  args->append(allocation_site);
