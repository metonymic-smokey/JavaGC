...
static void pass_arg4(MacroAssembler* masm, Register arg) {
  masm->push(arg);
}

static void pass_arg5(MacroAssembler* masm, Register arg) {
  masm->push(arg);
}
...
Register MacroAssembler::tlab_refill(Label& retry,
  ...
  if(TraceObjectsAllocations) {
      call_VM(top, CAST_FROM_FN_PTR(address, EventsSharedRuntime::fire_filler_alloc), top);
  }
  ...
  if(TraceObjectsAllocations) {
      call_VM(noreg, CAST_FROM_FN_PTR(address, EventsSharedRuntime::fire_tlab_alloc));
  }
