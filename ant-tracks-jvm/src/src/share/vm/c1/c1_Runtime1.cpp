...
JRT_ENTRY(void, Runtime1::new_instance(JavaThread* thread, Klass* klass, AllocationSiteIdentifier allocation_site))
  ...
  bool deopt = TraceObjectsAllocations && !AllocationSites::is_consistent(allocation_site);
  patch_alloc_slow_with_allocation_site(thread, EVENTS_C1_ALLOC_SLOW, allocation_site, klass) {
...
JRT_ENTRY(void, Runtime1::new_type_array(JavaThread* thread, Klass* klass, jint length, AllocationSiteIdentifier allocation_site))
  ...
  bool deopt = TraceObjectsAllocations && !AllocationSites::is_consistent(allocation_site);
  patch_alloc_slow_with_allocation_site(thread, EVENTS_C1_ALLOC_SLOW, allocation_site, TypeArrayKlass::cast(klass)) {
...
JRT_ENTRY(void, Runtime1::new_object_array(JavaThread* thread, Klass* array_klass, jint length, AllocationSiteIdentifier allocation_site))
  ...
  objArrayOop obj;
  bool deopt = TraceObjectsAllocations && !AllocationSites::is_consistent(allocation_site);
  patch_alloc_slow_with_allocation_site(thread, EVENTS_C1_ALLOC_SLOW, allocation_site, array_klass) {
...
JRT_ENTRY(void, Runtime1::new_multi_array(JavaThread* thread, Klass* klass, intptr_t rank_and_allocation_site, jint* dims))
  NOT_PRODUCT(_new_multi_array_slowcase_cnt++;)
  int rank = (int) (rank_and_allocation_site & 0xFFFFFFFF);
  AllocationSiteIdentifier allocation_site = (AllocationSiteIdentifier) ((rank_and_allocation_site >> (HeapWordSize - SIZE_OF_ALLOCATION_SITE_IDENTIFIER_SMALL) * 8) & 0xFFFF);

  assert(klass->is_klass(), "not a class");
  assert(rank >= 1, "rank must be nonzero");
  Handle holder(THREAD, klass->klass_holder()); // keep the klass alive
  oop obj;
  patch_alloc_slow_with_allocation_site(thread, EVENTS_C1_ALLOC_SLOW_DEVIANT_TYPE, allocation_site, klass) {
        obj = ArrayKlass::cast(klass)->multi_allocate(rank, dims, CHECK);
  }
  thread->set_vm_result(obj);
JRT_END
