...
#define RUNTIME1_STUBS(stub, last_entry) \
  ...
  stub(fire_obj_alloc_fast_with_event) \
  stub(fire_obj_alloc_fast_with_oop) \
  stub(store_alloc_site)             \
  stub(store_alloc_site_inlined)     \
...
class Runtime1: public AllStatic {
 ...
 private:
  ...
  static void new_instance    (JavaThread* thread, Klass* klass, AllocationSiteIdentifier allocation_site);
  static void new_type_array  (JavaThread* thread, Klass* klass, jint length, AllocationSiteIdentifier allocation_site);
  static void new_object_array(JavaThread* thread, Klass* klass, jint length, AllocationSiteIdentifier allocation_site);
  static void new_multi_array (JavaThread* thread, Klass* klass, intptr_t rank_and_allocation_site, jint* dims);
