...
// LIR_OpAllocObj
    case lir_alloc_object:
    {
      ...
      if(opAllocObj->trace_stub() != NULL)       do_stub(opAllocObj->trace_stub());
      break;
    }
...
// LIR_OpAllocArray;
    case lir_alloc_array: {
      ...
      if(opAllocArray->trace_stub() != NULL)          do_stub(opAllocArray->trace_stub());
      break;
    }
...
void LIR_OpAllocObj::emit_code(LIR_Assembler* masm) {
  ...
  CodeStub* s = stub();
  s->set_bci(source()->printable_bci());
  masm->append_code_stub(s);
  CodeStub* ts = trace_stub();
  if(ts != NULL) {
      ts->set_bci(source()->printable_bci());
      masm->append_code_stub(ts);
  }
}
...
void LIR_List::allocate_object(LIR_Opr dst, LIR_Opr t1, LIR_Opr t2, LIR_Opr t3, LIR_Opr t4,
                               int header_size, int object_size, LIR_Opr klass, bool init_check, CodeStub* stub, CodeStub* trace_stub, Klass* type) {
...
void LIR_List::allocate_array(LIR_Opr dst, LIR_Opr len, LIR_Opr t1,LIR_Opr t2, LIR_Opr t3,LIR_Opr t4, BasicType type, LIR_Opr klass, CodeStub* stub, CodeStub* trace_stub, Klass* allocated_type) {
