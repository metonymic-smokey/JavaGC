...
jint Arguments::parse_each_vm_init_arg(const JavaVMInitArgs* args,
  ...
  for (int index = 0; index < args->nOptions; index++) {
    ...
    if (match_option(option, "-verbose", &tail)) {
      ...
      } else if (!strcmp(tail, ":jit")) {
        FLAG_SET_CMDLINE(bool, PrintCompilation, true);
      } else if (!strcmp(tail, ":all")) {
        FLAG_SET_CMDLINE(bool, TraceClassLoading, true);
        FLAG_SET_CMDLINE(bool, TraceClassUnloading, true);
        FLAG_SET_CMDLINE(bool, PrintGC, true);
        FLAG_SET_CMDLINE(bool, PrintJNIResolving, true);
        FLAG_SET_CMDLINE(bool, PrintCompilation, true);
        FLAG_SET_CMDLINE(bool, PrintTraceObjects, true);
      }
    ...
    } else if (match_option(option, "-XX:+TraceObjectsMaxRunTimePerformance", &tail) && strlen(tail) == 0) {
        FLAG_SET_CMDLINE(bool, TraceObjectsCompressTrace, false);
        FLAG_SET_CMDLINE(bool, TraceObjectsCompressTraceAdaptively, false);
        FLAG_SET_CMDLINE(bool, TraceObjectsSaveAllocationSitesForSyncs, false);
        FLAG_SET_CMDLINE(bool, TraceObjectsAllocationStackTraces, false); 
    } else if (match_option(option, "-XX:+TraceObjectsPointers", &tail) && strlen(tail) == 0) {
        FLAG_SET_CMDLINE(bool, TraceObjectsPointers, true);
        FLAG_SET_CMDLINE(bool, TraceObjectsGCRoots, true);
        FLAG_SET_CMDLINE(bool, TraceObjects, true);
    } else if (match_option(option, "-XX:+TraceObjectsPointersDirtyOnly", &tail) && strlen(tail) == 0) {
        FLAG_SET_CMDLINE(bool, TraceObjectsPointersDirtyOnly, true);
        FLAG_SET_CMDLINE(bool, TraceObjectsPointers, true);
        FLAG_SET_CMDLINE(bool, TraceObjectsGCRoots, true);
        FLAG_SET_CMDLINE(bool, TraceObjects, true);
    } else if (match_option(option, "-XX:+ReuseUnloadedAllocationSites", &tail) && strlen(tail) == 0) { 
        FLAG_SET_CMDLINE(bool, ReuseUnloadedAllocationSites, true); //default = true
    } else if (match_option(option, "-XX:-ReuseUnloadedAllocationSites", &tail) && strlen(tail) == 0) {
        FLAG_SET_CMDLINE(bool, ReuseUnloadedAllocationSites, false); //default = true
    } else if (match_option(option, "-XX:+ExtendAllocationSiteIdRange", &tail) && strlen(tail) == 0) {
        FLAG_SET_CMDLINE(bool, ExtendAllocationSiteIdRange, true); //default = true
    } else if (match_option(option, "-XX:-ExtendAllocationSiteIdRange", &tail) && strlen(tail) == 0) {
        FLAG_SET_CMDLINE(bool, ExtendAllocationSiteIdRange, false); //default = true
    } else if (match_option(option, "-XX:TraceObjectsBufferSize=", &tail)) {
        julong size = 0;
        if (parse_memory_size(tail, &size, 1) != arg_in_range) {
            jio_fprintf(defaultStream::error_stream(),
                    "Invalid thread local buffer size: %s\n", option->optionString);
            return JNI_EINVAL;
        }
        FLAG_SET_CMDLINE(uintx, TraceObjectsBufferSize, (uintx) size);
    } else if (match_option(option, "-XX:TraceObjectsMaxTraceSize=", &tail)) {
        julong size = 0;
        if (parse_memory_size(tail, &size, 0) != arg_in_range) {
            jio_fprintf(defaultStream::error_stream(),
                    "Invalid trace restart size: %s\n", option->optionString);
            return JNI_EINVAL;
        }
        FLAG_SET_CMDLINE(uint64_t, TraceObjectsMaxTraceSize, (uint64_t) size);
  ...
  if(TraceObjectsSaveAllocationSites && FLAG_IS_DEFAULT(UseBiasedLocking)) {
    FLAG_SET_CMDLINE(bool, UseBiasedLocking, false);
  }
