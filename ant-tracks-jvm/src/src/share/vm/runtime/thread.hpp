...
#include "../prims/EventPointers.hpp"
#include "../prims/EventsRuntime.hpp"
#include "../prims/EventsWriter.hpp"
#include "../prims/AllocationTracingDefinitions.hpp"
#include "../prims/AllocationSites.hpp"
#include "../prims/EventBuffer.hpp"
#include "../prims/AllocationTracingSelfMonitoringTLS.hpp"
...
class Thread: public ThreadShadow {
 ...
 private:
  ...
  // Private thread-local objectmonitor list - a simple cache organized as a SLL.
    // required for allocation tracing
 public:
...
#undef TLAB_FIELD_OFFSET
 ...
 public:
  ...
  // AllocationTracing
  public:
    inline EventBuffer* get_event_buffer() { return &_event_buffer; }
    void flush_event_buffer(bool get_new = false);
    static ByteSize offset_of_event_buffer_bottom() { return (byte_offset_of(Thread, _event_buffer) + byte_offset_of(EventBuffer, bottom)); } 
    static ByteSize offset_of_event_buffer_top() { return (byte_offset_of(Thread, _event_buffer) + byte_offset_of(EventBuffer, top)); }
    static ByteSize offset_of_event_buffer_end() { return (byte_offset_of(Thread, _event_buffer) + byte_offset_of(EventBuffer, end)); } 
    
    EventType get_patched_event_type() { return _patched_event_type; }
    AllocationSiteIdentifier get_patched_allocation_site() { return _patched_allocation_site; }
    void set_patched_allocation_info(EventType event_type, AllocationSiteIdentifier allocation_site) { _patched_event_type = event_type; _patched_allocation_site = allocation_site; }
    
    AllocationTracingSelfMonitoringTLSData* get_self_monitoring_data() { return &_self_monitoring_data; }
    static ByteSize offset_of_self_monitoring_data() { return byte_offset_of(Thread, _self_monitoring_data); }
    
    inline EventPointers* get_event_obj_ptrs() { return _event_obj_ptrs; }
    void add_event_obj_ptr(oop ptr);
  private:
    EventBuffer _event_buffer;
    EventPointers* _event_obj_ptrs;
    
    AllocationSiteIdentifier _patched_allocation_site;
    EventType _patched_event_type;
    
    AllocationTracingSelfMonitoringTLSData _self_monitoring_data;
};
...
class EventsWorkerThread: public Thread{
public: 
       virtual void run();
       // Constructor
       EventsWorkerThread(EventsWriter* writer);
       bool is_running() { return running; }
       void interrupt_and_join();
       elapsedTimer* get_work_timer() { return &work_timer; }
       bool is_idle() { return idle; };
private:
       Monitor* termination_lock;
       EventsWriter* events_writer;
       volatile bool running;
       volatile int interrupting;
       volatile bool idle;
       elapsedTimer work_timer;
};
//WIP 
class CompressionThread: public Thread{
public:
    virtual void run();
    //function to heuristically evaluate to compress or not
    static bool evaluateCompression();
    static const int CONCURRENT_PROCESSINGS = 10;
    static int max_compression_threads;
    static bool add_thread_in_process(Thread* thread);
    static bool remove_thread_in_process(Thread* thread);
    static bool add_CompressionThread(CompressionThread* thread);
    static CompressionThread* get_CompressionThread(int idx);
    static void clear_CompressionThreads();
    static void destroyCompressionAssets();
    static void setWriteTime(double time);
    static void compressionBufferAdded(size_t buffer_size);
    static bool isCompressionUsed();
    static void initCompression();
    CompressionThread(const char* name, bool master);
    bool is_Idle() { return idle; };
    static void interrupt_and_join();
private:
    static Monitor* termination_lock;
    static int running;
    static int interrupting;
    static int nOfProcesses;
    static Thread** in_process;
    static Monitor* mod_lock;
    static CompressionThread** compression_Threads;
    static double encodingTime;
    static double writeTime;
    static size_t compressionFilling;
    static double fillRateTimeStart;
    static double fillRate;
    static size_t fillRateCounter;
    volatile bool idle;
