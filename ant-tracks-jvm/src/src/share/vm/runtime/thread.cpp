...
#include "../prims/EventBuffers.hpp"
...
Thread::Thread() {
  ...
  if(TraceObjects){
      _event_buffer = EventBuffer_create_dummy();
      EventBuffer_set_owner(&_event_buffer, this);
      _patched_allocation_site = is_GC_task_thread() ? ALLOCATION_SITE_IDENTIFIER_VM_GC : ALLOCATION_SITE_IDENTIFIER_VM_INTERNAL;
      _patched_event_type = EVENTS_ALLOC_SLOW;
      _self_monitoring_data = AllocationTracingSelfMonitoring::create_TLS();
      if(TraceObjectsPointers){
          _event_obj_ptrs = new EventPointers();
      }
  }
}
...
Thread::~Thread() { 
  if(TraceObjects){
      if(AllocationTracing::is_active()) {
          flush_event_buffer();
          self_monitoring(1) {
              AllocationTracingSelfMonitoring::report_TLS(this);
          }
      }
      if(TraceObjectsPointers){
          delete _event_obj_ptrs;
          _event_obj_ptrs = NULL;
      }
  }
...
void Thread::flush_event_buffer(bool get_new) {
    // assertion removed as we support flushing of buffers from gc threads only.
    //assert(Thread::current() == this || SafepointSynchronize::is_at_safepoint(), "someone else is trying to flush without a safepoint (might result in concurrency issues)");
    if(EventBuffer_is_valid(&_event_buffer)) {
        size_t buffer_size = EventBuffer_get_length(&_event_buffer);
        if(buffer_size > 0) {
            // if we do not guard this, we might add empty buffers to the list during thread-deallocation ...
            self_monitoring(1) {
                this->_self_monitoring_data.size += buffer_size * sizeof(jint);
                this->_self_monitoring_data.flushes++;
            }
            if(TraceObjectsAsyncIO) {
                if(TraceObjectsCompressTrace || TraceObjectsCompressTraceAdaptively) {
                    EventBuffers::enqueue_to_compression(_event_buffer);
                    CompressionThread::compressionBufferAdded(buffer_size);
                } else {
                    EventBuffers::enqueue_to_flush_queue(_event_buffer);
                }
            } else {
                EventBuffers::flush(&_event_buffer);
                EventBuffers::enqueue_to_free_list(_event_buffer);
            }
        } else {
            EventBuffer_reset(&_event_buffer);
            EventBuffers::enqueue_to_free_list(_event_buffer);
        }
    }
    if(get_new) {
        EventBuffers::dequeue_from_free_list(&_event_buffer);
#ifdef ASSERT
        assert(_event_buffer.owner == NULL, "buffer is already owned by someone else");
        assert(Thread::current() == this, "who is getting a new buffer for another thread?");
#endif
    } else {
        _event_buffer = EventBuffer_create_dummy();
    }
...
void Thread::add_event_obj_ptr(oop p) {
    _event_obj_ptrs->add(p);
}
...
EventsWorkerThread::EventsWorkerThread(EventsWriter* writer) : Thread() {
  termination_lock = new Mutex(Mutex::native, "EventsWorkerThread TerminationLock", true);
  events_writer = writer;
  work_timer = elapsedTimer();
  running = false;
  interrupting = 0;
  idle = false;
...
void EventsWorkerThread::interrupt_and_join() {
    SpinAcquire(&interrupting, "Interrupt Lock");
    
    //fetch pointer into local variable because terminating the thread may lead to deallocation
    Monitor* lock = termination_lock;
    lock->lock_without_safepoint_check();
    EventBuffers::finalize_flush_queue();
    while(running) {
        lock->wait(true);
    }
    lock->unlock();
    ...
    SpinRelease(&interrupting);
}

void EventsWorkerThread::run(){
    ...
    while(true){
        EventBuffer buffer;
        bool dequeued = EventBuffers::dequeue_from_flush_queue(&buffer, &idle);
        if(!dequeued){
            break;
        }
        self_monitoring_measure_time(1, &work_timer) {
            size_t buffer_size = EventBuffer_get_length(&buffer);
            double start = os::elapsedTime();
            events_writer->write_buffer(&buffer);
            CompressionThread::setWriteTime((os::elapsedTime()-start)/buffer_size);
	}
        EventBuffers::enqueue_to_free_list(buffer);
        EventSynchronization::check_trigger_rotate();
    }
    
    {
        MutexLockerEx l(termination_lock, true);
        running = false;
        termination_lock->notify();
    }
    
    assert(EventBuffer_get_length(get_event_buffer()) == 0, "we should not generate events");
    
    //make sure interrupt code has returned, otherwise it operates on invalid data if we start deleting
    SpinAcquire(&interrupting, "Interrupt Lock"); SpinRelease(&interrupting);
    
    delete this;
}

Thread** CompressionThread::in_process = NULL;
int CompressionThread::max_compression_threads = 0;
CompressionThread** CompressionThread::compression_Threads = NULL;
Monitor* CompressionThread::mod_lock = NULL;
int CompressionThread::nOfProcesses = 0;
//compression time per byte/bit
double CompressionThread::encodingTime = 0.0;           // time to encode per byte
//write time per byte/bit
double CompressionThread::writeTime = 0.0;              // time to write per byte
double CompressionThread::fillRateTimeStart = -1.0;     // filling start time
double CompressionThread::fillRate = -1.0;              // bytes per second added
size_t CompressionThread::fillRateCounter = 0;          // counting bytes for fillRate
size_t CompressionThread::compressionFilling = 0;       // amount of bytes in compression queue
int CompressionThread::running = 0;
Monitor* CompressionThread::termination_lock = NULL;
int CompressionThread::interrupting = 0;

CompressionThread::CompressionThread(const char* name, bool master) : Thread() {
  idle = false;
  this->master = master;
  if(fillRateTimeStart < 0){
      fillRateTimeStart = os::elapsedTime();
  }
  if (os::create_thread(this, os::eventw_thread)) {
      running++;
      assert(!DisableStartThread, "");
...
void CompressionThread::initCompression(){
    CompressionThread::in_process = (Thread**) calloc(sizeof(Thread*), CONCURRENT_PROCESSINGS);
    CompressionThread::max_compression_threads = os::processor_count();
    CompressionThread::compression_Threads = (CompressionThread**)calloc(sizeof(CompressionThread*), max_compression_threads);
    CompressionThread::mod_lock = new Monitor(Mutex::native, "CompressionProgressLock", true);
    CompressionThread::termination_lock = new Mutex(Mutex::native, "CompressionThread Termination", true);
}

void CompressionThread::interrupt_and_join() {
    SpinAcquire(&interrupting, "Interrupt Lock");
    //fetch pointer into local variable because terminating the thread may lead to deallocation
    Monitor* lock = termination_lock;
    lock->lock_without_safepoint_check();
    EventBuffers::finalize_compression_queue();
    while(running > 0) {
        lock->wait(true);
    }
    lock->unlock();
...
bool CompressionThread::isCompressionUsed(){
    return running > 0;
    
}

void CompressionThread::destroyCompressionAssets(){
    delete CompressionThread::mod_lock; 
    CompressionThread::mod_lock = NULL;
    free(CompressionThread::in_process);
    free(CompressionThread::compression_Threads);
    CompressionThread::in_process = NULL;
    CompressionThread::nOfProcesses = 0;
    CompressionThread::running = 0;
}

bool CompressionThread::evaluateCompression(){
    if(EventBuffers::compression_fill_level() > TraceObjectsCompressTraceAdaptiveThreshold){
            return EventBuffers::flush_queue_fill_level() > TraceObjectsCompressTraceAdaptiveThreshold;;
    }   
    if(encodingTime > 0 && encodingTime < writeTime){
        return true;
    }
    double r = fillRate*encodingTime;
    if(r <= 1.0){
        return true;
    }
    return EventBuffers::flush_queue_fill_level() > TraceObjectsCompressTraceAdaptiveThreshold;
...
void CompressionThread::setWriteTime(double time){
    if(time > 0){
        CompressionThread::writeTime = time;
    }
}

void CompressionThread::compressionBufferAdded(size_t buffer_size) {
    synchronized(mod_lock){
        CompressionThread::compressionFilling += buffer_size;
        double time = os::elapsedTime() - fillRateTimeStart;
        if(time >= 1.0){
                fillRate = fillRateCounter/time; //normalize to per second
                fillRateCounter = buffer_size;
                fillRateTimeStart = os::elapsedTime();
        }else{
            fillRateCounter+=buffer_size;
        }
...
bool CompressionThread::add_CompressionThread(CompressionThread* thread){
    int i = 0;
    while(i < max_compression_threads){
        if(compression_Threads[i] == NULL){
            compression_Threads[i] = thread;
            return true;
        }
        i++;
    }
...
void CompressionThread::clear_CompressionThreads(){
    int i = 0;
    while(i < max_compression_threads){
        compression_Threads[i] = NULL;
        i++;
    }
}

CompressionThread* CompressionThread::get_CompressionThread(int idx){
    if(idx >= 0 && idx < max_compression_threads) {
        return compression_Threads[idx];
    }
    return NULL;
}

bool CompressionThread::add_thread_in_process(Thread* thread){
    synchronized(mod_lock) {
        int i = 0;
        while(i < nOfProcesses){
            if(in_process[i]==thread){
                return false;
            }
            i++;
        }
        in_process[nOfProcesses] = thread;
        nOfProcesses++;
        
        synchronized_notify_all(mod_lock);
        return true;
    }
    HERE_BE_DRAGONS(false);
}

bool CompressionThread::remove_thread_in_process(Thread* thread){
    synchronized(mod_lock) {
        int i = 0;
        while(i < nOfProcesses){
            if(in_process[i]==thread){
                nOfProcesses--;
                in_process[i] = in_process[nOfProcesses];
                return true;
            }
            i++;
        }
...
void CompressionThread::run() {
    ...
    if(TraceObjectsCompressTraceUseLZW) {
        compressor = new LZWCompression(TraceObjectsCompressTraceLZWIndexCollection);
    } else if(TraceObjectsCompressTraceUseLZWQuyteTree) {
        compressor = new QuyteTreeCompression();
    } else {
        compressor = new QuyteTreeCompression();
    }
    
    EventBuffer event_buffer = EventBuffer_create_dummy();
    for(;;) {
        // Mode 0 = Normal event buffer -> Further processing
        // Mode 1 = EventBuffer is sync event buffer (flush without compression) or another buffer of the same thread is already in process by some compression thread () -> continue
        // Mode 2 = Peek failed, stop compression thread -> break
        int mode = 0;
        idle = true;
        critical {
            idle = false;
            if(!EventBuffers::peek_from_compression(&event_buffer, &idle)) {
                mode = 2;
            } else if(event_buffer.sync != Sync_None) {
                if(this->master) {
                    int i = 0;
                    while(i < max_compression_threads){
                        CompressionThread* thread = CompressionThread::get_CompressionThread(i);
                        if(thread != NULL && thread != this) {
                            while(!thread->is_Idle()) os::yield_all();
                        }
                        i++;
                    }
                    EventBuffers::dequeue_from_compression(&event_buffer);
                    EventBuffers::enqueue_to_flush_queue(event_buffer);
                }
                mode = 1;
            } else if(!(CompressionThread::add_thread_in_process(event_buffer.owner))) {
                mode = 1;
            } else {
                EventBuffers::dequeue_from_compression(&event_buffer);
                mode = 0;
        ...
        if(mode == 1) continue;
        if(mode == 2) break;
        Thread* owner = event_buffer.owner;
        size_t buffer_size = EventBuffer_get_length(&event_buffer);
        assert(event_buffer.sync == Sync_None,"sync buffer detected, should not happen");
        if(buffer_size == 0) {
            EventBuffer_reset(&event_buffer);
            EventBuffers::enqueue_to_free_list(event_buffer);
            CompressionThread::remove_thread_in_process(owner);
            continue;
        }
        bool must_compress = (TraceObjectsCompressTrace || (TraceObjectsCompressTraceAdaptively && CompressionThread::evaluateCompression()));
        if(!must_compress) {
            EventBuffers::enqueue_to_flush_queue(event_buffer); 
            CompressionThread::remove_thread_in_process(owner);
            continue;
        }
        EventBuffer compressed_buffer; 
        if(!EventBuffers::dequeue_from_free_list(&compressed_buffer)) {
            // Should not happen at all because free list is not finalized (=> always returns true)
            EventBuffers::enqueue_to_flush_queue(event_buffer); 
            CompressionThread::remove_thread_in_process(owner);
            continue;
        }
        EventBuffer_set_owner(&compressed_buffer, owner);
        compressed_buffer.sync = event_buffer.sync;
        bool compressed;
        // self_monitoring_measure_time(1, &Thread::current()->get_self_monitoring_data()->compressed_timer) {
        //    compressed = compressor->encode(&_event_buffer, &compressed_buffer);    
        // } //Potenzial Problem?!
        double start = os::elapsedTime();
        self_monitoring_measure_time(1, &this->get_self_monitoring_data()->compressed_timer) {
                compressed = compressor->encode(&event_buffer, &compressed_buffer);    
        }
        double duration = os::elapsedTime() - start;
        CompressionThread::encodingTime = CompressionThread::encodingTime <= 0.0 ? (duration/buffer_size) : ((CompressionThread::encodingTime + (duration/buffer_size))/2);
        
        if(compressed) {
            #ifdef COMPRESSION_PARANOID_ASSERTIONS
            // this is most likely broken, but ... who cares
            Decompression* decompressor = compressor->getDecompressor();
            unsigned char* decompressor_buffer = (unsigned char*) calloc(buffer_size, sizeof(jint));
            size_t decompressor_buffer_size = buffer_size;
            size_t decompressed_length = decompressor->decode((unsigned char*) EventBuffer_get_data(&compressed_buffer), EventBuffer_get_raw_length(&compressed_buffer), decompressor_buffer, decompressor_buffer_size * sizeof(jint));
            assert(compare((unsigned char*) EventBuffer_get_data(&event_buffer), EventBuffer_get_raw_length(&event_buffer), decompressor_buffer, decompressed_length), "compress and decompress does not yield original!");
            decompressor->reset();
            delete decompressor;
            decompressor = NULL;
            #endif
            self_monitoring(1){
                AllocationTracingSelfMonitoring::report_compressed_buffer(this);
            }
            EventBuffers::enqueue_to_flush_queue(compressed_buffer);
            EventBuffer_reset(&event_buffer);
            EventBuffers::enqueue_to_free_list(event_buffer);
        } else {
            EventBuffers::enqueue_to_flush_queue(event_buffer);
            EventBuffer_reset(&compressed_buffer);
            EventBuffers::enqueue_to_free_list(compressed_buffer);
        }
        CompressionThread::compressionFilling -= buffer_size;
        CompressionThread::remove_thread_in_process(owner);
    }
...
void JavaThread::run() {
  ...
  if(TraceObjects && TraceObjectsThreadEvents) {
      EventsRuntime::fire_thread_alive(this);
  }
...
void JavaThread::exit(bool destroy_vm, ExitType exit_type) {
  ...
  if(TraceObjects) {
      if(TraceObjectsThreadEvents) EventsRuntime::fire_thread_death(this);
      //do this here, because the following Threads::remove makes the thread invisible for us, so we are missing a buffer until the destructor is called (which may be too late if a GC starts at that very moment)
      flush_event_buffer(false);
  }
