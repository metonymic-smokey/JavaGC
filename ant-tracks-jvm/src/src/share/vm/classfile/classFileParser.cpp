...
instanceKlassHandle ClassFileParser::parseClassFile(Symbol* name,
  ...
  if(TraceObjects && TraceObjectsClassDefinitions) {
      SymbolsClassDefinition symbol;
      symbol.allocated_type_id = AllocatedTypes::get_allocated_type_id(this_klass());
      symbol.buffer = cfs->buffer();
      symbol.length = cfs->length();
      AllocationTracing::get_class_definitions_writer()->write_symbol(symbol);
  }
