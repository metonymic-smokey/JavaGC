...
#include "../prims/EventsInterpreterRuntime.hpp"
...
IRT_ENTRY(void, InterpreterRuntime::_new(JavaThread* thread, ConstantPool* pool, int index))
  ...
  patch_alloc_slow(thread, EVENTS_IR_ALLOC_SLOW, method(thread), bcp(thread), k_oop) {
...
IRT_ENTRY(void, InterpreterRuntime::newarray(JavaThread* thread, BasicType type, jint size))
  oop obj;
  patch_alloc_slow(thread, EVENTS_IR_ALLOC_SLOW, method(thread), bcp(thread), Universe::typeArrayKlassObj(type)) {
      obj = oopFactory::new_typeArray(type, size, CHECK);
  }
  thread->set_vm_result(obj);
IRT_END
...
IRT_ENTRY(void, InterpreterRuntime::anewarray(JavaThread* thread, ConstantPool* pool, int index, jint size))
  ...
  patch_alloc_slow(thread, EVENTS_IR_ALLOC_SLOW, method(thread), bcp(thread), klass->array_klass(thread)) {
...
IRT_ENTRY(void, InterpreterRuntime::multianewarray(JavaThread* thread, jint* first_size_address))
  ...
  oop obj;
  patch_alloc_slow(thread, EVENTS_IR_ALLOC_SLOW_DEVIANT_TYPE, method(thread), bcp(thread), klass) {
      obj = ArrayKlass::cast(klass)->multi_allocate(nof_dims, dims, CHECK);
  }
