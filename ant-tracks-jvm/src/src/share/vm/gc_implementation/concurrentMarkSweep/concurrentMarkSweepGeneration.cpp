...
#include "compactibleFreeListSpace.hpp"
...
// Tracing includes
#include "prims/EventsGCRuntime.hpp"
  ...
  _did_compact(false)
{
  ...
  if (TraceObjectsGC) {
    EventsRuntime::fire_space_creation(GENERATIONAL_GC_GEN_1_OLD_ID, _cmsSpace->bottom(), _cmsSpace->end());
...
void CMSCollector::direct_allocated(HeapWord* start, size_t size) {
  ...
  if (_collectorState >= Marking) {
    ...
    // temporarily turn off event fireing, as this is not an initialized object!
    _markBitMap.mark(start);           // object is live
    _markBitMap.mark(start + 1);       // object is potentially uninitialized?
    _markBitMap.mark(start + size - 1);// mark end of object
  }
...
ConcurrentMarkSweepGeneration::par_promote(int thread_num,
  ...
  if (TraceObjectsGC) {
    EventsGCRuntime::fire_gc_move_slow(old, obj);
  }
  return obj;
}
...
NOT_PRODUCT(
  ...
  if (should_compact) {
    ...
    if (TraceObjectsGC) {
      assert(!EventsGCRuntime::is_gc_active(), "not possible/state invariant");
      _gc_id = EventsGCRuntime::fire_gc_start(GCType_Major, GCCause::_no_cause_specified, false, false, true);
      //EventsGCRuntime::fire_gc_info(true, _gc_id);
    }
    do_compaction_work(clear_all_soft_refs);    
    if (TraceObjectsGC) {
        EventsGCRuntime::fire_gc_end(GCType_Major, _gc_id, GCCause::_no_cause_specified);
    }
    // can not be active now.
#ifdef ASSERT
    assert(!EventsGCRuntime::is_gc_active(), "not possible/state invariant");
#endif
    ...
    size_policy()->check_gc_overhead_limit(_young_gen->used(),
      ...
      if (TraceObjectsGC
            && first_state > Idling
            && first_state <= Sweeping /* gc_running */
            && should_start_over) {
        assert(EventsGCRuntime::is_gc_active(), "has to be");
        /* we scrap all our current work and start all over */
        if(first_state == Sweeping) {
            //EventsRuntime::fire_sync(true);
            //EventsGCRuntime::fire_gc_failed(GENERATIONAL_GC_GEN_1_OLD_ID);
        }
        //EventsGCRuntime::fire_gc_end(GCType_Minor, _gc_id, GCCause::_no_gc, true);
      }
...
void ConcurrentMarkSweepGeneration::shrink_by(size_t bytes) {
  ...
  if (TraceObjects) postpone_space_events {
      EventsGCRuntime::schedule_space_redefine(GENERATIONAL_GC_GEN_1_OLD_ID, _cmsSpace->bottom(), _cmsSpace->end(), false);
  }
}
...
bool ConcurrentMarkSweepGeneration::grow_by(size_t bytes) {
  ...
  if (result) {
    if (TraceObjects) {
        //do this early or other threads will race us and use the expanded memory region before we can send the event
        EventsGCRuntime::schedule_space_redefine(GENERATIONAL_GC_GEN_1_OLD_ID, _cmsSpace->bottom(), (HeapWord*)_virtual_space.high(), true);
    }
...
void CMSCollector::checkpointRootsInitial(bool asynch) {
  ...
  jint gc_id;
  if (TraceObjectsGC) {
    gc_id = EventsGCRuntime::fire_gc_start(GCType_Minor, GCCause::_cms_initial_mark, false, false, false);
  }
  ...
  if (TraceObjectsGC) {
    EventsGCRuntime::fire_gc_end(GCType_Minor, gc_id, GCCause::_cms_initial_mark);
    //_gc_id = EventsGCRuntime::fire_gc_start(GCType_Minor, GCCause::_cms_concurrent_mark, false, false);
  }
}
...
void CMSCollector::checkpointRootsFinal(bool asynch,
  ...
  jint gc_id;
  if (TraceObjectsGC) {
    //EventsGCRuntime::fire_gc_end(GCType_Minor, _gc_id, GCCause::_cms_concurrent_mark);
    gc_id = EventsGCRuntime::fire_gc_start(GCType_Minor, GCCause::_cms_final_remark, false, false, false);
  }
  ...
  if (TraceObjectsGC) {
    EventsGCRuntime::fire_gc_end(GCType_Minor, gc_id, GCCause::_cms_final_remark);
  }
}
...
void CMSCollector::sweepWork(ConcurrentMarkSweepGeneration* gen,
  ...
  if(TraceObjectsGC) {
    _gc_id = EventsGCRuntime::fire_gc_start(GCType_Minor, GCCause::_cms_sweeping, true, true, false);
    EventsGCRuntime::fire_gc_info(GENERATIONAL_GC_GEN_1_OLD_ID, _gc_id);
    EventsRuntime::fire_sync(false);
  }
...

  {
    SweepClosure sweepClosure(this, gen, &_markBitMap,
                            CMSYield && asynch);
    HeapWord* sweepLimit = gen->cmsSpace()->sweep_limit();
    gen->cmsSpace()->blk_iterate_careful(&sweepClosure);
    HeapWord* new_limit = gen->cmsSpace()->end();
    // We need to free-up/coalesce garbage/blocks from a
    // co-terminal free run. This is done in the SweepClosure
    // destructor; so, do not remove this scope, else the
    // end-of-sweep-census below will be off by a little bit.
    if (TraceObjectsGC && sweepLimit < new_limit) { // there are objects to be saved
      HeapWord* sweepLastAddr = sweepClosure.get_last_address(); // last address seen by the closure (may be above sweepLimit)
      size_t sweepLastSize = sweepClosure.get_last_size();
      assert(sweepLastAddr != NULL, "just checking");
      
      // Special case when SweepClosure did not help the loop finish (see SweepClosure::do_blk_careful)
      sweepLastAddr += sweepLastSize;
      
      assert(sweepLimit <= sweepLastAddr && sweepLastAddr < new_limit, "math doesn't check out");
      AntTracksClosure atc(this, gen, &_markBitMap);
      gen->cmsSpace()->blk_iterate_careful_from(&atc, sweepLastAddr);
    }
  }
  gen->cmsSpace()->sweep_completed();
  gen->cmsSpace()->endSweepFLCensus(sweep_count());
  if (TraceObjectsGC) {
    EventsGCRuntime::fire_gc_end(GCType_Minor, _gc_id, GCCause::_cms_sweeping, false, true);
  }
...
size_t SweepClosure::do_blk_careful(HeapWord* addr) {
  ...
  if (addr >= _limit) { // we have swept up to or past the limit: finish up
    ...
    _lastSize = 0; // we don't want to skip too far ahead
  ...
  if (fc->is_free()) {
    ...
    if (TraceObjectsGC) {
      ...
      EventsGCRuntime::fire_gc_keep_alive((oop) addr, (int) res);
    }
    debug_only(_sp->verifyFreeLists());
    NOT_PRODUCT(
        _numObjectsLive++;
...
void SweepClosure::do_yield_work(HeapWord* addr) {
  if(TraceObjectsGC) {
    ...
    EventsGCRuntime::fire_gc_interrupt(_collector->_gc_id, addr);
  }
  ...
  if(TraceObjectsGC) {
    EventsGCRuntime::fire_gc_continue(_collector->_gc_id, addr);
  }
}
...
AntTracksClosure::AntTracksClosure(CMSCollector* collector, ConcurrentMarkSweepGeneration* g,
               CMSBitMap* bitMap) :
  _collector(collector),
  _g(g),
  _bitMap(bitMap),
  _limit(g->cmsSpace()->end()),
  _sp(g->cmsSpace()),
  _freelistLock(_sp->freelistLock()),
  _last_addr(NULL)
{}

size_t AntTracksClosure::do_garbage_chunk(FreeChunk* fc) {
  HeapWord* const addr = (HeapWord*) fc;
  return CompactibleFreeListSpace::adjustObjectSize(oop(addr)->size());
}

size_t AntTracksClosure::do_already_free_chunk(FreeChunk* fc) {
  assert(false, "should not be called");
  return 0;
}

size_t AntTracksClosure::do_live_chunk(FreeChunk* fc) {
  HeapWord* addr = (HeapWord*) fc;
  size_t size;
  assert(_bitMap->isMarked(addr), "Tautology for this control point");
  if (_bitMap->isMarked(addr + 1)) {
  ...
  if (TraceObjectsGC) {
    ...
    EventsGCRuntime::fire_gc_keep_alive((oop) addr, (int) size);
  }
  return size;
}

size_t AntTracksClosure::do_blk_careful(HeapWord* addr) {
  FreeChunk* fc = (FreeChunk*)addr;
  size_t res;
  ...
  if (fc->is_free()) {
    ...
    // Chunk is fresh garbage
    HeapWord* next = _bitMap->getNextMarkedWordAddress(addr);
    res = pointer_delta(next, addr);
    assert(_limit <= next || _bitMap->isMarked(addr + res), "sanity");
  } else {
    // Chunk that is alive.
    res = do_live_chunk(fc);
  }
  assert(res > 0, "progress check");
  _last_addr = addr;
  return res;
}
