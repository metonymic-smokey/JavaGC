...
public:
  jint _gc_id; // used by AntTracks to remember the id returned by gc_start
 private:
...
class SweepClosure: public BlkClosureCareful {
 ...
 public:
  ...
  HeapWord*    get_last_address() { return _lastAddress; }
  size_t       get_last_size() { return _lastSize; }
  size_t       do_blk_careful(HeapWord* addr);
  void         print() const { print_on(tty); }
  void         print_on(outputStream *st) const;
};

class AntTracksClosure: public BlkClosureCareful {
  CMSCollector*                  _collector;  // collector doing the work
  ConcurrentMarkSweepGeneration* _g;    // Generation being swept
  CompactibleFreeListSpace*      _sp;   // Space being swept
  HeapWord*                      _limit;
  ...
  size_t do_already_free_chunk(FreeChunk* fc);
  
  size_t do_live_chunk(FreeChunk* fc);
public:
    AntTracksClosure(CMSCollector* collector, ConcurrentMarkSweepGeneration* g,
               CMSBitMap* bitMap);
    size_t do_blk_careful(HeapWord* addr);
};
