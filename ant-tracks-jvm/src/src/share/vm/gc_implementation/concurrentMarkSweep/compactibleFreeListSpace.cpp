...
void CompactibleFreeListSpace::blk_iterate_careful_from(BlkClosureCareful* sc, HeapWord* addr) {
  assert_lock_strong(freelistLock());
  HeapWord *cur, *limit;
  for (cur = addr, limit = end(); cur < limit;
       cur += sc->do_blk_careful(cur));
}
...
oop CompactibleFreeListSpace::promote(oop obj, size_t obj_size) {
  ...
  if (res != NULL) {
    ...
    if (TraceObjectsGC) {
      EventsGCRuntime::fire_gc_move_slow(obj, oop(res), (int) obj->size());
    }
  }
  return oop(res);
}
