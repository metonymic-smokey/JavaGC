...
class PSYoungGen : public CHeapObj<mtGC> {
 ...
 public:
  ...
  MutableSpace* survivor_1_space() const { return from_space()->bottom() < to_space()->bottom() ? from_space() : to_space(); }
  MutableSpace* survivor_2_space() const { return from_space()->bottom() < to_space()->bottom() ? to_space() : from_space(); }
