...
void PSPromotionLAB::flush() {
  ...
  if(TraceObjectsGC) {
      ...
      EventsGCRuntime::fire_plab_flushed(bottom(), filler_oop);
      AllocationSiteIdentifier allocation_site = ALLOCATION_SITE_IDENTIFIER_VM_GC;
      if(TraceObjectsSaveAllocationSites) {
          AllocationSiteStorage::store(thread, filler_oop, allocation_site);
      }
      EventsRuntime::fire_obj_alloc_slow(thread, filler_oop, allocation_site, EVENTS_ALLOC_SLOW);
  }
