...
public:
  ...
  public:
    ...
    HeapWord* deferred_obj_src() const { return _partial_obj_src; }
    ...
    void set_deferred_obj_addr(HeapWord* addr) { _partial_obj_addr = addr; _partial_obj_src = NULL; }
    void set_deferred_obj_src(HeapWord* addr)  { _partial_obj_src = addr; }
...
class PSParallelCompact : AllStatic {
 ...
 public:
  ...
  template <class T> static inline void mark_and_push(ParCompactionManager* cm,
                                                      T* p);
  template <class T> static inline void adjust_pointer(T* p, bool root=false);
...
template <class T>
inline void PSParallelCompact::adjust_pointer(T* p, bool root) {
  T heap_oop = oopDesc::load_heap_oop(p);
  if (!oopDesc::is_null(heap_oop)) {
    oop obj     = oopDesc::decode_heap_oop_not_null(heap_oop);
    ...
    if (new_obj != NULL) {
      ...
      if(TraceObjectsPointers){
          if(!root) {
              Thread::current()->add_event_obj_ptr(new_obj);
          }
      }
    }
  } else if(TraceObjectsPointers){
       if(!root) {
...
inline void UpdateOnlyClosure::do_addr(HeapWord* addr)
{
  ...
  if(TraceObjectsPointersDirtyOnly){
      ...
      const jbyte* start = table->byte_for_const(addr);
      const jbyte* end = table->byte_for_const(addr + oop(addr)->size());
      do {
  ...
  if(TraceObjectsPointers) Thread::current()->get_event_obj_ptrs()->set_meta(EVENTS_GC_KEEP_ALIVE, (uintptr_t) addr, true, 0, -1, !dirty);
  oop(addr)->update_contents(compaction_manager());
  
  if(TraceObjectsGC && !TraceObjectsPointers) _moves.fire(oop(addr), oop(addr), oop(addr)->size()); 
  else if(TraceObjectsPointers) Thread::current()->get_event_obj_ptrs()->flush();
}
...
public:
  ...
  virtual IterationStatus do_addr(HeapWord* addr, size_t size) {
    ...
    do {
      _start_array->allocate_block(addr);
      if(TraceObjectsGC) {
          EventsGCRuntime::fire_gc_filler_alloc(oop(addr));
      }
      addr += oop(addr)->size();
    } while (addr < end);
