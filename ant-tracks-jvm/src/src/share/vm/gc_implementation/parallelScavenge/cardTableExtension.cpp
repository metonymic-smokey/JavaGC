  ...
  for (jbyte* slice = start_card; slice < end_card; slice += slice_width) {
    ...
    while (current_card < worker_end_card) {
      ...
      if (first_unclean_card < worker_end_card) {
        ...
        // AntTracks: actually, all objects are dirty here?... since they are scanned, see previous comment
        if (interval != 0) {
          while (p < to) {
            ...
            if(TraceObjectsPointers) {
                Thread::current()->get_event_obj_ptrs()->set_meta(EVENTS_GC_UPDATE_PTR_PREMOVE, (uintptr_t) (HeapWord*) m, true);
            }
            m->push_contents(pm);
