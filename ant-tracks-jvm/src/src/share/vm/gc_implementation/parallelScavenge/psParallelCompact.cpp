...
void PSParallelCompact::AdjustPointerClosure::do_oop(oop* p)       { adjust_pointer(p, true); }
void PSParallelCompact::AdjustPointerClosure::do_oop(narrowOop* p) { adjust_pointer(p, true); }
                                          ...
                                          size_t words)
{
  ...
  if(TraceObjectsGC) {
      for (HeapWord* p = start; p < start + words; p += oop(p)->size()) {
          EventsGCRuntime::fire_gc_filler_alloc(oop(p));
      }
  }
  for (HeapWord* p = start; p < start + words; p += oop(p)->size()) {
...
PSParallelCompact::provoke_split_fill_survivor(SpaceId id)
{
  ...
  if (space->is_empty()) {
    ...
    while (b + obj_len <= t) {
      ...
      if(TraceObjectsGC) {
        EventsGCRuntime::fire_gc_filler_alloc(oop(b));
      }
...
PSParallelCompact::provoke_split(bool & max_compaction)
{
  ...
  if (live_words < region_size * 2) {
    ...
    while (fill_words > 0) {
      ...
      if(TraceObjectsGC) {
        EventsGCRuntime::fire_gc_filler_alloc(oop(cur_addr));
      }
...
void PSParallelCompact::fill_dense_prefix_end(SpaceId id)
{
  ...
  if (dead_space_crosses_boundary(region, dense_prefix_bit)) {
    ...
    if(TraceObjectsGC) {
        EventsGCRuntime::fire_gc_filler_alloc(oop(obj_beg));
    }
...
bool PSParallelCompact::invoke_no_policy(bool maximum_heap_compaction) {
  ...
  {
    int id;
    SlowAllocPatcher patcher;
    if(TraceObjectsGC) {
        id = EventsGCRuntime::fire_gc_start(GCType_Major, heap->gc_cause());
        patcher = SlowAllocPatcher(Thread::current(), EVENTS_ALLOC_SLOW, ALLOCATION_SITE_IDENTIFIER_VM_GC);
    } else {
        patcher = SlowAllocPatcher();
    }
    ...
    if(TraceObjectsGC) {
      EventsGCRuntime::fire_gc_end(GCType_Major, id, heap->gc_cause());
    }
  }
...
bool PSParallelCompact::absorb_live_data_from_eden(PSAdaptiveSizePolicy* size_policy,
  ...
  if (unused_words > 0) {
    ...
    if(TraceObjectsGC) {
        for (HeapWord* p = unused_start; p < unused_start + unused_words; p += oop(p)->size()) {
            EventsGCRuntime::fire_gc_filler_alloc(oop(p));
        }
    }
  }
...
void PSParallelCompact::compact() {
  //FlushAndSyncEventBufferOnLeave _(true); //flush at the beginning because prev phases might have allocated something
  if(TraceObjectsGC) {
      EventsGCRuntime::sync();
  }
  // trace("5");
...
PSParallelCompact::update_and_deadwood_in_dense_prefix(ParCompactionManager* cm,
  ...
  if (beg_addr < end_addr) {
    //FlushAndSyncEventBufferOnLeave _();
  ...
  for (cur_region = beg_region; cur_region < end_region; ++cur_region) {
    ...
    if (addr != NULL) {
      ...
      if(TraceObjectsPointers) {
          HeapWord* from = cur_region->deferred_obj_src();
          bool dirty = true;
          if(TraceObjectsPointersDirtyOnly){
              CardTableExtension* table = (CardTableExtension*) Universe::heap()->barrier_set();
              const jbyte* start = table->byte_for_const(from);
              const jbyte* end = table->byte_for_const(from + ((int) cm->mark_bitmap()->obj_size(from)));
              do {
                  dirty = table->card_is_dirty(*start);
                  start++;
              } while(!dirty && start <= end); 
          }
          
          assert(from != NULL, "Unknown from address");         
          Thread::current()->get_event_obj_ptrs()->set_meta(EVENTS_GC_MOVE_SLOW, (uintptr_t) addr, true, (uintptr_t) from, -1, !dirty);
      }
      ...
      if(TraceObjectsGC) {
          if(!TraceObjectsPointers){
              HeapWord* from = cur_region->deferred_obj_src();
              assert(from != NULL, "Unknown from address");
              EventsGCRuntime::fire_gc_move_slow(oop(from), oop(addr), (int) cm->mark_bitmap()->obj_size(from));
          } else {
...
void PSParallelCompact::fill_region(ParCompactionManager* cm, size_t region_idx)
{
  ...
  do {
    ...
    if (status == ParMarkBitMap::would_overflow) {          
      ...
      //if(TraceObjectsGC && EventSynchronization::is_synchronizing() && closure.words_remaining() < (size_t) oopDesc::header_size()) {
      if(TraceObjectsGC) {
          region_ptr->set_deferred_obj_src(closure.source());
      }
...
ParMarkBitMap::IterationStatus MoveAndUpdateClosure::copy_until_full()
{
  ...
  if(TraceObjectsGC && (!EventSynchronization::is_synchronizing() || words_remaining() >= (size_t) oopDesc::header_size())) {
    //at this point the beginning of an object is copied because it does not fit entirely into the block
    /*
     * AntTracks: TODO remove this event as we can handle it afterwards
    */
    //EventsGCRuntime::fire_gc_move_slow((oop) source(), (oop) destination(), (int) bitmap()->obj_size(source()));
  }
...
void MoveAndUpdateClosure::copy_partial_obj()
{
  ...
  if(TraceObjectsGC) {
      //do nothing here because the end of an object is copied and the event has already been fired when the start was copied
  }
  update_state(words);
}
...
MoveAndUpdateClosure::do_addr(HeapWord* addr, size_t words) {
  ...
  jlong offset = source() - destination(); // HeapWords!
  
  bool dirty = true;
  if(TraceObjectsPointersDirtyOnly){
        CardTableExtension* table = (CardTableExtension*) Universe::heap()->barrier_set();
        const jbyte* start = table->byte_for_const(_source); 
        const jbyte* end = table->byte_for_const(_source + words);
          do {
            dirty = table->card_is_dirty(*start);
            start++;            
        } while(!dirty && start <= end); 
  ...
  if(TraceObjectsGC && !TraceObjectsPointers) {
    _gc_move.fire(oop(source()), oop(destination()), words);
  }

  oop moved_oop = (oop) destination();
  if(TraceObjectsPointers) Thread::current()->get_event_obj_ptrs()->set_meta(EVENTS_GC_MOVE_SLOW, (uintptr_t) destination(), true, (uintptr_t) source(), -1, !dirty);  
  moved_oop->update_contents(compaction_manager());
  if(TraceObjectsPointers) Thread::current()->get_event_obj_ptrs()->flush(); 
