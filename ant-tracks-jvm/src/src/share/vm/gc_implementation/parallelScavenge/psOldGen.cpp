...
void PSOldGen::initialize_work(const char* perf_data_name, int level) {
  ...
  if(TraceObjects) {
      EventsRuntime::fire_space_creation(PARALLEL_GC_OLD_ID, object_space()->bottom(), object_space()->end());
...
bool PSOldGen::expand_by(size_t bytes) {
  ...
  if (result) {
    ...
    if(TraceObjects) {
      EventsGCRuntime::schedule_space_redefine(PARALLEL_GC_OLD_ID, object_space()->bottom(), object_space()->end(), true);
    }
  }
...
void PSOldGen::shrink(size_t bytes) {
  ...
  if (size > 0) {
    ...
    if(TraceObjects) {
      EventsGCRuntime::schedule_space_redefine(PARALLEL_GC_OLD_ID, object_space()->bottom(), object_space()->end(), false);
    }
