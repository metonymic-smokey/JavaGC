  ...
  virtual void do_object(oop obj) {
    if (obj->is_forwarded()) {
      ...
      if(TraceObjectsSaveAllocationSites) {
          AllocationSiteStorage::store(Thread::current(), obj, ALLOCATION_SITE_IDENTIFIER_VM_GC_SCAVENGE_ZOMBIE);
      }
    }
  }
};
...
bool PSScavenge::invoke_no_policy() {
...

  {
    ...
    uint survivor_id;
    if(TraceObjectsGC) {
        id = EventsGCRuntime::fire_gc_start(GCType_Minor, heap->gc_cause());
        ...
        EventsGCRuntime::fire_gc_info(PARALLEL_GC_EDEN_ID, id);
        EventsGCRuntime::fire_gc_info(survivor_id, id);
    ...
    PSPromotionManager* promotion_manager = PSPromotionManager::vm_thread_promotion_manager();
    {
      ...
      if(TraceObjectsGC) {
          EventSynchronization::enqueue_synchronization_tasks_for_parallel_old_gc(q, active_workers);
      }
    ...
    if (!promotion_failure_occurred) {
      ...
      if (UseAdaptiveSizePolicy) postpone_space_events {
    ...
    if(TraceObjectsGC) {
        if(promotion_failure_occurred) {
            ...
            EventsGCRuntime::fire_gc_failed(PARALLEL_GC_EDEN_ID);
            EventsGCRuntime::fire_gc_failed(survivor_id);
        }
        EventsGCRuntime::fire_gc_end(GCType_Minor, id, heap->gc_cause(), promotion_failure_occurred);
        // self_monitoring(3) AllocationTracingSelfMonitoring::print_statistics();
    }
    if(TraceObjectsPointersDirtyOnly) {
        CardTableExtension* table = (CardTableExtension*) Universe::heap()->barrier_set();
        ParallelScavengeHeap* heap = (ParallelScavengeHeap*) Universe::heap();
        table->clear(heap->young_gen()->to_space()->region());   
        table->clear(heap->young_gen()->from_space()->region());
        table->clear(heap->young_gen()->eden_space()->region());      
    }
