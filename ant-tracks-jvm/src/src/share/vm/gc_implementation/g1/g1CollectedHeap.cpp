...
HeapWord* G1CollectedHeap::humongous_obj_allocate(size_t word_size, AllocationContext_t context) {
  ...
  if (first != G1_NO_HRM_INDEX) {
    ...
    if(PrintTraceObjectsHumongous) {
        AllocationTracing::log_humongous_allocation(word_size * HeapWordSize);
    }
  }
...
HeapWord* G1CollectedHeap::allocate_new_tlab(size_t word_size) {
  ...
  HeapWord* result = attempt_allocation(word_size, &dummy_gc_count_before, &dummy_gclocker_retry_count);
  if(TraceObjectsAllocations && result != NULL) {
      EventsRuntime::fire_tlab_alloc(result, word_size * HeapWordSize);
  }
  return result;
}
...
bool G1CollectedHeap::do_collection(bool explicit_gc,
  ...
  if(TraceObjects) {
      ...
      if(cause == GCCause::_g1_inc_collection_pause) {
          // This should not happen here because major GCs cannot be of type g1_inc_collection
          if(g1_policy()->gcs_are_young()) {
              cause = GCCause::_g1_inc_collection_pause_young;
          } else {
              cause = GCCause::_g1_inc_collection_pause_mixed;
          }
      }
      id = EventsGCRuntime::fire_gc_start(GCType_Major, cause);
      // cnINFO -- start major
//      gclog_or_tty->print("=== GC START MAJOR\n");
      G1MarkSweep::gc_id = id;
  }
  
  {
    ...
    if(TraceObjects) {
        // cnINFO -- end major
//        gclog_or_tty->print("=== GC END   MAJOR\n");
      GCCause::Cause cause = gc_cause();
      if(cause == GCCause::_g1_inc_collection_pause) {
          // Remark: This should not happen here because major collections cannot be of type g1_inc_collection
          if(g1_policy()->gcs_are_young()) {
              cause = GCCause::_g1_inc_collection_pause_young;
          } else {
              cause = GCCause::_g1_inc_collection_pause_mixed;
      ...
      EventsGCRuntime::fire_gc_end(GCType_Major, id, cause);
    }
...
// cnINFO FirePointerEventsClosure
class FirePointerEventsClosure : public HeapRegionClosure {
  protected:
    uint32_t _eventsCtr;

    bool is_collected_region(HeapRegion* target) {
      return target->in_collection_set();
    }
    
public:
    FirePointerEventsClosure() {
        _eventsCtr = 0;
    }
    
    ~FirePointerEventsClosure() {
        //gclog_or_tty->print("=== Sent %" PRIu32 " %s events\n", _eventsCtr, "POSTMOVE");
    }

    bool do_region(HeapRegion* region, HeapWord* start, HeapWord* end) {
      if(region->continuesHumongous()) {
          return true;
      }
      
      // ignore evacuated into regions
      if (!region->sendPostEvents) {
          return true;
      }
      bool is_collected = is_collected_region(region);

      // Skip regions which have been collected before 
      // we are in postmove and encounter a collected region
      if (is_collected)
        return true;
        
      Thread* self = Thread::current();
      for(HeapWord* obj = region->block_start(start); obj < end && obj < region->top(); obj = obj + oop(obj)->size()) {
        
        oop obj_ptr = oop(obj);
        int objSize = obj_ptr->size();
        bool isObjArray = obj_ptr->is_objArray();
        bool isTypeArry = obj_ptr->is_typeArray();
        bool isGCmarked = obj_ptr->is_gc_marked();

        self->get_event_obj_ptrs()->set_meta(EVENTS_GC_UPDATE_PTR_POSTMOVE, (uintptr_t) obj, true);
        if(oop(obj)->is_array()) {
          if(oop(obj)->is_objArray()) {
            if(UseCompressedOops) {
              narrowOop* first = (narrowOop*) objArrayOop(obj)->base();
              narrowOop* end = first + objArrayOop(obj)->length();
              for(narrowOop* element = first; element < end; element++) {
                self->add_event_obj_ptr(oopDesc::decode_heap_oop(*element));
                _eventsCtr++;
              }
            } else {
              oop* first = (oop*) objArrayOop(obj)->base();
              oop* end = first + objArrayOop(obj)->length();
          ...
          InstanceKlass* klass = (InstanceKlass*)oop(obj)->klass();
          OopMapBlock* map           = klass->start_of_nonstatic_oop_maps();
          OopMapBlock* const end_map = map + klass->nonstatic_oop_map_count();
          if (UseCompressedOops) {
            while (map < end_map) {
              narrowOop* p = (narrowOop*)oop(obj)->obj_field_addr<narrowOop>(map->offset());
              narrowOop* const end = p + (map->count());
              while (p < end) {
                self->add_event_obj_ptr(oopDesc::decode_heap_oop(*p));
                _eventsCtr++;
                ++p;
              }
              ++map;
            ...
            while (map < end_map) {
              oop* p = (oop*)oop(obj)->obj_field_addr<oop>(map->offset());
              oop* const end = p + (map->count());
              while (p < end) {
                self->add_event_obj_ptr(*p); 
                _eventsCtr++;
                ++p;                                    
              }                                             
              ++map;                                                             
        ...
        self->get_event_obj_ptrs()->flush();
      }
      return false;
    }

    bool doHeapRegion(HeapRegion* r) {
      do_region(r, r->bottom(), r->top());
      // cnINFO return value of do_region(...) is ignored...
      return false;
    }
};
...
G1CollectedHeap::do_collection_pause_at_safepoint(double target_pause_time_ms) {
  ...
  if(TraceObjectsGC) {
      ...
      id = EventsGCRuntime::fire_gc_start(GCType_Minor, cause);
      // cnINFO -- start minor
//      gclog_or_tty->print("=== GC START MINOR (%s)\n", GCCause::to_string(gc_cause()));
      G1MarkSweep::gc_id = id;
  }
  ...
  // Inner scope for scope based logging, timers, and stats collection
  {
    ...
    { // Call to jvmpi::post_class_unload_events must occur outside of active GC
...

      {
        ...
        if(TraceObjectsGC) {
          size_t hrCount = _hrm.length();
//          gclog_or_tty->print("G1 Collected regions: [");
          if (TraceObjectsPointers) {
            for (uint idx = 0; idx < num_regions(); idx++) {
            HeapRegion* hr = _hrm.at(idx);
              assert(hr->hrm_index() == idx, "assumption");
              if (hr->in_collection_set()) {
                EventsGCRuntime::fire_gc_info(idx, id);
                hr->sendPostEvents = false;
  //              gclog_or_tty->print("%u(%s) ", idx,
  //                  hr->is_old()? "old" :
  //                  hr->is_young()? "young" :
  //                  hr->is_eden()? "eden" :
  //                  hr->is_survivor()? "survivor" :
  //                    "other");
              } else {
            hr->sendPostEvents = true;
          }
            }
          }
//          gclog_or_tty->print("]\n");
        ...
        // cnINFO evacuate_collection_set
        // Actually do the work...
//        for (uint idx = 0; idx < num_regions(); idx++) {
//          HeapRegion* hr = _hrm.at(idx);
//          ObjectInRegionCountClosure cl = ObjectInRegionCountClosure();
//          hr->object_iterate(&cl);
//          gclog_or_tty->print("[Space#%u objectcount: %"PRIu32"]\n", hr->hrm_index(), cl.count);
//        }
//        gclog_or_tty->print("\n\n");
        evacuate_collection_set(evacuation_info);

        if (TraceObjectsGC) {
          EventsRuntime::fire_sync(true); // make sure parsed heap is as close to real heap as possible

          if(evacuation_failed()) {
          EventsRuntime::fire_sync(true);
          // cnINFO -- failed
          gclog_or_tty->print("= FAILED");
          for(HeapRegion* region = G1CollectedHeap::heap()->g1_policy()->collection_set(); region != NULL; region = region->next_in_collection_set()) {
            if(region->evacuation_failed()) {
              EventsGCRuntime::fire_gc_failed(region->hrm_index());
              hasFailed = true;
            }
          }
        }
        ...
        if(!TraceObjectsPointersDirtyOnly && TraceObjectsPointers) {
          EventsRuntime::fire_sync(true);
          FirePointerEventsClosure cl = FirePointerEventsClosure();
          _hrm.iterate(&cl);
        }
...
//          ObjectInRegionCountClosure cl = ObjectInRegionCountClosure();
//          hr->object_iterate(&cl);
//          gclog_or_tty->print("[Space#%u objectcount: %"PRIu32" | firstEvacAddress=%"PRIiPTR"]\n", hr->hrm_index(), cl.count, (intptr_t) hr->firstAddrEvacuatedTo);
//        }
//        gclog_or_tty->print("\n\n");
  ...
  if(TraceObjectsGC) {
    // cnINFO -- end minor
//    gclog_or_tty->print("=== GC END   MINOR (%s)\n", GCCause::to_string(gc_cause()));
      GCCause::Cause cause = gc_cause();
      if(cause == GCCause::_g1_inc_collection_pause) {
          if(g1_policy()->gcs_are_young()) {
              cause = GCCause::_g1_inc_collection_pause_young;
      ...
      EventsGCRuntime::fire_gc_end(GCType_Minor, id, cause, hasFailed);
  }
...
G1CollectedHeap::handle_evacuation_failure_par(G1ParScanThreadState* _par_scan_state,
  ...
  if (forward_ptr == NULL) {
    ...
    // cnINFO clear obj_ptrs evac failure
    if (TraceObjectsPointers) {
        EventPointers* lists = Thread::current()->get_event_obj_ptrs();
        size_t size = lists->get()->get_size();
        if(size > 0){
            // not sure if pointers of the object need to be tracked here... 
            // just clear for the moment
            lists->clear();
        }
    }
...
void G1CollectedHeap::evacuate_collection_set(EvacuationInfo& evacuation_info) {
...

  {
    ...
    if(TraceObjectsGC) {
        EventSynchronization::enqueue_synchronization_tasks_for_g1_gc(workers());
    }
...
void G1CollectedHeap::free_region(HeapRegion* hr,
  ...
  if (TraceObjectsGC) {
      EventsRuntime::fire_space_release(hr->hrm_index());
  }
}
...
public:
  ...
  bool doHeapRegion(HeapRegion* r) {
    ...
    if (r->is_empty()) {
      // Check if object was not free before GC and now is empty -> Release event
      // If object was in freelist before GC and is now empty -> Handle as it was never removed from freelist during GC
      if (TraceObjectsGC && !r->is_free()) {
        EventsRuntime::fire_space_release(r->hrm_index());
      }
