...
void HeapRegionDCTOC::walk_mem_region(MemRegion mr,
  ...
  if (!g1h->is_obj_dead(oop(cur), _hr)) {
    ...
    //    Thread::current()->get_event_obj_ptrs()->set_meta(EVENTS_GC_UPDATE_PTR_PREMOVE, (uintptr_t) (HeapWord*) cur, true);
    //}
...
void HeapRegion::set_startsHumongous(HeapWord* new_top, HeapWord* new_end) {
  ...
  if(TraceObjects) {
      EventsRuntime::fire_space_alloc(hrm_index(), SPACE_MODE_HUMONGOUS_START, OLD_SPACE);
  }
}
...
void HeapRegion::set_continuesHumongous(HeapRegion* first_hr) {
  ...
  if(TraceObjects) {
    EventsRuntime::fire_space_alloc(hrm_index(), SPACE_MODE_HUMONGOUS_CONTINUES, OLD_SPACE);
  }
}
...
void HeapRegion::initialize(MemRegion mr, bool clear_space, bool mangle_space) {
  ...
  if(TraceObjects) {
      EventsRuntime::fire_space_creation(_hrm_index, bottom(), end());
  }
