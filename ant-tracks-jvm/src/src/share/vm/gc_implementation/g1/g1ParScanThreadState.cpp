  ...
  if (forward_ptr == NULL) {
    ...
    if(TraceObjectsGC) {
        if(TraceObjectsPointers) {
            HeapRegion* const toRegion = _g1h->heap_region_containing_raw(obj_ptr);
            toRegion->sendPostEvents = false;
#ifdef ASSERT
            /* This assertion code is here to enable per evacuated object assertions at gc_end
             * every region which has firstAddrEvacuatedTo set to anything other NULL, we expect
             * to have dealt with every object in [firstAddrEvacuatedTo, end()) */
            if(TraceObjectsGCEnableParanoidAssertions) {
              intptr_t val;
              val = Atomic::cmpxchg((intptr_t) obj_ptr, (volatile intptr_t*) (&toRegion->firstAddrEvacuatedTo), (intptr_t) NULL);
              while (obj_ptr < (HeapWord*) val/* evacuateTo address is before currently remembered first AddrEvacuatedTo */) {
                val = Atomic::cmpxchg((intptr_t) obj_ptr, (volatile intptr_t*) (&toRegion->firstAddrEvacuatedTo), (intptr_t) val);
              }
              /* either we are the first to evacuate into this region OR
               * we evacuate an object above firstAddrEvacuatedTo OR
               * a thread overtook us and we had to adjust firstAddrEvacuatedTo */
              assert((HeapWord*) val == NULL ||
                   (HeapWord*) val < obj_ptr ||
                   (HeapWord*) val == obj_ptr, "the fuq?");
            }
#endif
            EventType event_type = fast? EVENTS_GC_MOVE_FAST : EVENTS_GC_MOVE_SLOW;  // AntTracks
            bool dirty = true;
            SpaceType to_space = dest_state.is_young() ? SURVIVOR_SPACE : OLD_SPACE; // AntTracks
            Thread::current()->get_event_obj_ptrs()->set_meta(event_type, (uintptr_t) ((HeapWord*) oop(obj_ptr)), true, (uintptr_t) (HeapWord*) old, to_space, !dirty);
            //gclog_or_tty->printf("Set meta for %p\n", oop(obj_ptr));
            //Manual iteration - just for debugging...
            //add_obj_pointers(obj_ptr);
        } else {
            if(fast) {
                EventsGCRuntime::fire_gc_move_fast(old, oop(obj_ptr), dest_state.is_young() ? SURVIVOR_SPACE : OLD_SPACE);
            } else {
                EventsGCRuntime::fire_gc_move_slow(old, oop(obj_ptr));
            }
        }
    }
