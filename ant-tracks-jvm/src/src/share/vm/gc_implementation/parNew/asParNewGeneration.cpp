...
void ASParNewGeneration::resize_spaces(size_t requested_eden_size,
  ...
  HeapWord *old_eden_bottom = eden()->bottom(), *old_eden_end = eden()->end();
  HeapWord *old_from_bottom = from()->bottom(), *old_from_end = from()->end();
  HeapWord   *old_to_bottom =   to()->bottom(),   *old_to_end =   to()->end();
  ...
  if (TraceObjects) {
      EventsGCRuntime::schedule_space_redefine(GENERATIONAL_GC_GEN_0_EDEN_ID,       eden()->bottom(), eden()->end(), old_eden_bottom, old_eden_end);
      EventsGCRuntime::schedule_space_redefine(GENERATIONAL_GC_GEN_0_SURVIVOR_1_ID, from()->bottom(), from()->end(), old_from_bottom, old_from_end);
      EventsGCRuntime::schedule_space_redefine(GENERATIONAL_GC_GEN_0_SURVIVOR_2_ID,   to()->bottom(),   to()->end(),   old_to_bottom,   old_to_end);
  }
