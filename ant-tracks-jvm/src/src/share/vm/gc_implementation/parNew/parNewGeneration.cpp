  ...
  if (!_to_space_full) {
    ...
    if (word_sz * 100 <
      ...
      if (buf_space != NULL) {
        ...
        if (TraceObjectsGC) {
          EventsGCRuntime::fire_plab_alloc(buf_space, plab->word_sz() * HeapWordSize);
        }
...
void ParNewGeneration::collect(bool   full,
  ...
  uint survivor_id;
  if (TraceObjectsGC) {
      gc_id = EventsGCRuntime::fire_gc_start(GCType_Minor, gch->gc_cause(), false, false, false);
      ...
      EventSynchronization::do_synchronization_tasks_for_generation_n(1); // TODO could be parallel
  }
  ...
  if (!collection_attempt_is_safe()) {
    ...
    if (TraceObjectsGC) {
        ...
        EventsGCRuntime::fire_gc_end(GCType_Minor, gc_id, gch->gc_cause(), true);
    }
    return;
  }
  ...
  if (TraceObjectsGC) {
    ...
    EventsGCRuntime::fire_gc_end(GCType_Minor, gc_id, gch->gc_cause(), gc_failed);
  }
}
...
oop ParNewGeneration::copy_to_survivor_space_avoiding_promotion_undo(
  ...
  if (forward_ptr == NULL) {
    ...
    if (TraceObjectsGC && !promoted && !_promotion_failed) {
      assert(from()->is_in(old) || eden()->is_in(old), "neither from from-space nor eden-space?");
      assert(to()->is_in(new_obj), "not in to-space!");
      EventsGCRuntime::fire_gc_move_slow(old, new_obj);
    }
