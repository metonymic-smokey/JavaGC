...
void Parse::do_multianewarray() {
  ...
  if(TraceObjectsAllocations) {
      SingleCallSiteIterator call_sites = SingleCallSiteIterator((Method*) iter().method()->constant_encoding(), iter().cur_bci());
      allocation_site = intcon((int) AllocationSites::method_to_allocation_site(&call_sites, (Klass*) array_klass->constant_encoding()));
  } else {
      allocation_site = intcon(ALLOCATION_SITE_IDENTIFIER_UNKNOWN);
  }
  allocation_site = _gvn.transform(new (C) CastIINode(allocation_site, TypeInt::INT));
