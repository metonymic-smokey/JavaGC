...
bool OptoRuntime::generate(ciEnv* env) {
  ...
  gen(env, EventsC2Runtime::fire_obj_alloc_fast_Java, EventsC2RuntimeTypes::fire_obj_alloc_fast_Type, EventsC2Runtime::fire_obj_alloc_fast, 0, true, false, false);
  gen(env, EventsC2Runtime::fire_obj_alloc_fast_unknown_Java, EventsC2RuntimeTypes::fire_obj_alloc_fast_Type, EventsC2Runtime::fire_obj_alloc_fast_unknown, 0, true, false, false);
...
// object allocation
JRT_BLOCK_ENTRY(void, OptoRuntime::new_instance_C(Klass* klass, AllocationSiteIdentifier allocation_site, JavaThread* thread))
  JRT_BLOCK;
#ifndef PRODUCT
  ...
  if (!HAS_PENDING_EXCEPTION) {
    ...
    Handle holder(THREAD, klass->klass_holder()); // keep the klass alive
    oop result;
    patch_alloc_slow_relaxed(thread, EVENTS_C2_ALLOC_SLOW, allocation_site, klass) {
...
// array allocation
JRT_BLOCK_ENTRY(void, OptoRuntime::new_array_C(Klass* array_type, AllocationSiteIdentifier allocation_site, int len, JavaThread *thread))
  JRT_BLOCK;
#ifndef PRODUCT
  ...
  patch_alloc_slow_relaxed(thread, EVENTS_C2_ALLOC_SLOW, allocation_site, array_type)
...
JRT_BLOCK_ENTRY(void, OptoRuntime::new_array_nozero_C(Klass* array_type, AllocationSiteIdentifier allocation_site, int len, JavaThread *thread))
  JRT_BLOCK;
#ifndef PRODUCT
  SharedRuntime::_new_array_ctr++;            // new array requires GC
#endif
...
JRT_ENTRY(void, OptoRuntime::multianewarray2_C(Klass* elem_type, AllocationSiteIdentifier allocation_site, int len1, int len2, JavaThread *thread))
#ifndef PRODUCT
  ...
  oop obj;
  patch_alloc_slow_with_allocation_site(thread, EVENTS_C2_ALLOC_SLOW_DEVIANT_TYPE, allocation_site, ArrayKlass::cast(elem_type))
...
JRT_ENTRY(void, OptoRuntime::multianewarray3_C(Klass* elem_type, AllocationSiteIdentifier allocation_site, int len1, int len2, int len3, JavaThread *thread))
#ifndef PRODUCT
  ...
  oop obj;
  patch_alloc_slow_with_allocation_site(thread, EVENTS_C2_ALLOC_SLOW_DEVIANT_TYPE, allocation_site, ArrayKlass::cast(elem_type))
      obj = ArrayKlass::cast(elem_type)->multi_allocate(3, dims, THREAD);
  deoptimize_caller_frame(thread, HAS_PENDING_EXCEPTION);
  thread->set_vm_result(obj);
JRT_END

// multianewarray for 4 dimensions
JRT_ENTRY(void, OptoRuntime::multianewarray4_C(Klass* elem_type, AllocationSiteIdentifier allocation_site, int len1, int len2, int len3, int len4, JavaThread *thread))
#ifndef PRODUCT
  ...
  oop obj;
  patch_alloc_slow_with_allocation_site(thread, EVENTS_C2_ALLOC_SLOW_DEVIANT_TYPE, allocation_site, ArrayKlass::cast(elem_type))
      obj = ArrayKlass::cast(elem_type)->multi_allocate(4, dims, THREAD);
  deoptimize_caller_frame(thread, HAS_PENDING_EXCEPTION);
  thread->set_vm_result(obj);
JRT_END

// multianewarray for 5 dimensions
JRT_ENTRY(void, OptoRuntime::multianewarray5_C(Klass* elem_type, AllocationSiteIdentifier allocation_site, int len1, int len2, int len3, int len4, int len5, JavaThread *thread))
#ifndef PRODUCT
  ...
  oop obj;
  patch_alloc_slow_with_allocation_site(thread, EVENTS_C2_ALLOC_SLOW_DEVIANT_TYPE, allocation_site, ArrayKlass::cast(elem_type))
      obj = ArrayKlass::cast(elem_type)->multi_allocate(5, dims, THREAD);
  deoptimize_caller_frame(thread, HAS_PENDING_EXCEPTION);
  thread->set_vm_result(obj);
JRT_END

JRT_ENTRY(void, OptoRuntime::multianewarrayN_C(Klass* elem_type, arrayOopDesc* dims, AllocationSiteIdentifier allocation_site, JavaThread *thread))
  assert(check_compiled_frame(thread), "incorrect caller");
  assert(elem_type->is_klass(), "not a class");
  ...
  oop obj;
  patch_alloc_slow_with_allocation_site(thread, EVENTS_C2_ALLOC_SLOW_DEVIANT_TYPE, allocation_site, ArrayKlass::cast(elem_type))
      obj = ArrayKlass::cast(elem_type)->multi_allocate(len, c_dims, THREAD);
  deoptimize_caller_frame(thread, HAS_PENDING_EXCEPTION);
  thread->set_vm_result(obj);
JRT_END
...
const TypeFunc *OptoRuntime::new_instance_Type() {
  ...
  fields[TypeFunc::Parms+1] = TypeInt::SHORT; //allocation site bci
...
const TypeFunc *OptoRuntime::multianewarrayN_Type() {
  ...
  fields[TypeFunc::Parms+2] = TypeInt::SHORT;   // allocation site
  const TypeTuple *domain = TypeTuple::make(TypeFunc::Parms+3, fields);
