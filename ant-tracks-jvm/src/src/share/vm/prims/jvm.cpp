...
JVM_ENTRY(jobject, JVM_Clone(JNIEnv* env, jobject handle))
  ...
  patch_alloc_slow_with_allocation_site(Thread::current(), EVENTS_ALLOC_SLOW, ALLOCATION_SITE_IDENTIFIER_JVM__JAVA_LANG_OBJECT__CLONE, klass())
  if (obj->is_array()) {
  ...
  if(TraceObjectsSaveAllocationSites) {
      AllocationSiteStorage::store(Thread::current(), new_obj_oop, ALLOCATION_SITE_IDENTIFIER_JVM__JAVA_LANG_OBJECT__CLONE);
  }
...
JVM_ENTRY(jobject, JVM_AllocateNewObject(JNIEnv *env, jobject receiver, jclass currClass, jclass initClass))
 ...
 methodHandle m (THREAD,
  ...
  Handle obj;
  patch_alloc_slow_with_allocation_site(Thread::current(), EVENTS_ALLOC_SLOW, ALLOCATION_SITE_IDENTIFIER_JVM__JAVA_IO_OBJECT_INPUT_STREAM__ALLOCATE_NEW_OBJECT, java_lang_Class::as_Klass(curr_mirror))
      obj = curr_klass->allocate_instance_handle(CHECK_NULL);
...
JVM_ENTRY(jobject, JVM_AllocateNewArray(JNIEnv *env, jobject obj, jclass currClass, jint length))
  ...
  patch_alloc_slow_with_allocation_site(Thread::current(), EVENTS_ALLOC_SLOW, ALLOCATION_SITE_IDENTIFIER_JVM__JAVA_IO_OBJECT_INPUT_STREAM__ALLOCATE_NEW_ARRAY, k)
...
JVM_ENTRY(jobject, JVM_NewArray(JNIEnv *env, jclass eltClass, jint length))
  ...
  oop result;
  patch_alloc_slow_relaxed(Thread::current(), EVENTS_ALLOC_SLOW, ALLOCATION_SITE_IDENTIFIER_JVM__JAVA_LANG_REFLECT_ARRAY__NEW_ARRAY, NULL)
      result = Reflection::reflect_new_array(element_mirror, length, CHECK_NULL);
  return JNIHandles::make_local(env, result);
JVM_END
...
JVM_ENTRY(jobject, JVM_NewMultiArray(JNIEnv *env, jclass eltClass, jintArray dim))
  ...
  oop result;
  patch_alloc_slow_relaxed(Thread::current(), EVENTS_ALLOC_SLOW, ALLOCATION_SITE_IDENTIFIER_JVM__JAVA_LANG_REFLECT_ARRAY__NEW_MULTI_ARRAY, NULL)
      result = Reflection::reflect_new_multi_array(element_mirror, typeArrayOop(dim_array), CHECK_NULL);
  return JNIHandles::make_local(env, result);
JVM_END
...
JVM_ENTRY(jstring, JVM_InternString(JNIEnv *env, jstring str))
  ...
  oop result;
  patch_alloc_slow_relaxed(Thread::current(), EVENTS_ALLOC_SLOW, ALLOCATION_SITE_IDENTIFIER_JVM__JAVA_LANG_STRING__INTERN, NULL)
     result = StringTable::intern(string, CHECK_NULL);
  return (jstring) JNIHandles::make_local(env, result);
JVM_END
...
JVM_ENTRY(jobject, JVM_InvokeMethod(JNIEnv *env, jobject method, jobject obj, jobjectArray args0))
  ...
  if (thread->stack_available((address) &method_handle) >= JVMInvokeMethodSlack) {
    ...
    oop result;
    patch_alloc_slow_relaxed(Thread::current(), EVENTS_ALLOC_SLOW, ALLOCATION_SITE_IDENTIFIER_JVM__JAVA_LANG_REFLECT_METHOD__INVOKE, NULL)
        result = Reflection::invoke_method(method_handle(), receiver, args, CHECK_NULL);
...
JVM_ENTRY(jobject, JVM_NewInstanceFromConstructor(JNIEnv *env, jobject c, jobjectArray args0))
  ...
  objArrayHandle args(THREAD, objArrayOop(JNIHandles::resolve(args0)));
  oop result;
  patch_alloc_slow_relaxed(Thread::current(), EVENTS_ALLOC_SLOW, ALLOCATION_SITE_IDENTIFIER_JVM__JAVA_LANG_REFLECT_CONSTRUCTOR__NEW_INSTANCE, NULL)
      result = Reflection::invoke_constructor(constructor_mirror, args, CHECK_NULL);
  jobject res = JNIHandles::make_local(env, result);
  if (JvmtiExport::should_post_vm_object_alloc()) {
    JvmtiExport::post_vm_object_alloc(JavaThread::current(), result);
  }
