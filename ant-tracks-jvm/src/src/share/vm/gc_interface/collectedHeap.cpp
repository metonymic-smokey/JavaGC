...
HeapWord* CollectedHeap::allocate_from_tlab_slow(KlassHandle klass, Thread* thread, size_t size) {
    assert(thread == Thread::current(), "Someone else is allocating the tlab");
...
CollectedHeap::fill_with_array(HeapWord* start, size_t words, bool zap)
{
  ...
  if(TraceObjectsAllocations) {
      ...
      if(TraceObjectsSaveAllocationSites) {
          AllocationSiteStorage::store(thread, oop(start), allocation_site);
      }
      EventsRuntime::fire_obj_alloc_slow(thread, oop(start), allocation_site, EVENTS_ALLOC_SLOW);
  }
}
