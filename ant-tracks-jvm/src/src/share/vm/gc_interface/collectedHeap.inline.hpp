...
#include "prims/AllocationSiteStorage.hpp"
...
oop CollectedHeap::obj_allocate(KlassHandle klass, int size, TRAPS) {
  ...
  if(TraceObjectsAllocations) {
      // Instance mirror objects already handled in instanceMirrorKlass#allocate_instance
      if(!oop(obj)->is_instanceMirror()) {
        Thread* thread = Thread::current();
        AllocationSiteIdentifier allocation_site = thread->get_patched_allocation_site();
        if(TraceObjectsSaveAllocationSites) {
            AllocationSiteStorage::store(thread, oop(obj), allocation_site);
        }
        EventsRuntime::fire_obj_alloc_slow(thread, oop(obj), allocation_site, thread->get_patched_event_type(), size);
      }
  }
  return (oop)obj;
}
