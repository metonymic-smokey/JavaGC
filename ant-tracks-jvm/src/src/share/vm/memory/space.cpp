  ...
  for (; bottom < top; bottom += _sp->block_size(bottom)) {
    ...
    if (_sp->block_is_obj(bottom) &&
      ...
      //if(TraceObjectsPointers) {
         // Thread::current()->get_event_obj_ptrs()->set_meta(EVENTS_GC_UPDATE_PTR_PREMOVE, (uintptr_t) (HeapWord*) bottom, true);
     // }
...
bool CompactibleSpace::insert_deadspace(size_t& allowed_deadspace_words,
  ...
  if (allowed_deadspace_words >= deadlength) {
    ...
    //SlowAllocPatcher _ = TraceObjectsGC ? SlowAllocPatcher(Thread::current(), EVENTS_ALLOC_SLOW, ALLOCATION_SITE_IDENTIFIER_VM_GC_DEAD_SPACE) : SlowAllocPatcher();
    CollectedHeap::fill_with_object(q, deadlength);
    oop(q)->set_mark(oop(q)->mark()->set_marked());
    if(TraceObjectsSaveAllocationSites) {
        //resetting the mark destroys saved allocation sites, reinitialize
        //but we cannot store because it must be marked and we would have to displace the header ...
        //AllocationSiteStorage::store(Thread::current(), oop(q), ALLOCATION_SITE_IDENTIFIER_VM_GC_DEAD_SPACE);
        //assert(AllocationSiteStorage::load(Thread::current(), oop(q)) != ALLOCATION_SITE_IDENTIFIER_UNKNOWN, "just checking if resetting the mark here destroys our information");
    }
...
void ContiguousSpace::allocate_temporary_filler(int factor) {
  ...
  if (size >= (size_t)align_object_size(array_header_size)) {
    ...
    if(TraceObjectsAllocations) {
        ...
        AllocationSiteIdentifier allocation_site = ALLOCATION_SITE_IDENTIFIER_ARRAY_FILLER;
        if(TraceObjectsSaveAllocationSites) {
            AllocationSiteStorage::store(thread, oop(t), allocation_site);
        }
        EventsRuntime::fire_obj_alloc_slow(thread, oop(t), allocation_site, EVENTS_ALLOC_SLOW);
    }
  } else {
    ...
    if(TraceObjectsAllocations) {
        Thread* thread = Thread::current();
        AllocationSiteIdentifier allocation_site = ALLOCATION_SITE_IDENTIFIER_OBJECT_FILLER;
        if(TraceObjectsSaveAllocationSites) {
            AllocationSiteStorage::store(thread, oop(obj), allocation_site);
        }
        EventsRuntime::fire_obj_alloc_slow(thread, oop(obj), allocation_site, EVENTS_ALLOC_SLOW);
    }
  }
}
