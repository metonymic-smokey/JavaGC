...
oop Generation::promote(oop obj, size_t obj_size) {
  ...
  if (result != NULL) {
    ...
    if(TraceObjectsGC) {
        EventsGCRuntime::fire_gc_move_slow(obj, oop(result), (int) obj_size);
    }
    return oop(result);
  } else {
    GenCollectedHeap* gch = GenCollectedHeap::heap();
...
bool CardGeneration::expand(size_t bytes, size_t expand_bytes) {
  ...
  if(TraceObjects) {
      //do not do anything here because all is done in helper methods ...
  }
...
void OneContigSpaceCardGeneration::collect(bool   full,
  ...
  jint gc_id;
  if(TraceObjectsGC) {
      gc_id = EventsGCRuntime::fire_gc_start(GCType_Major, gch->gc_cause());
  }
  ...
  if(TraceObjectsGC) {
    EventsGCRuntime::fire_gc_end(GCType_Major, gc_id, gch->gc_cause());
  }
}
...
bool OneContigSpaceCardGeneration::grow_by(size_t bytes) {
  ...
  if (result) {    
    if(TraceObjects) {
        ...
        EventsGCRuntime::schedule_space_redefine(GENERATIONAL_GC_GEN_1_OLD_ID, _the_space->bottom(), (HeapWord*)_virtual_space.high(), true);
    }
...
void OneContigSpaceCardGeneration::shrink_by(size_t bytes) {
  ...
  if(TraceObjects) postpone_space_events {
      EventsGCRuntime::schedule_space_redefine(GENERATIONAL_GC_GEN_1_OLD_ID, _the_space->bottom(), _the_space->end(), false);
  }
}
