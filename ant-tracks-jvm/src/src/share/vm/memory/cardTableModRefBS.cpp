...
//void CardTableModRefBS::printCardTable(bool major) {
//       FILE *f = fopen("cardTable.txt", "a");
////       jbyte cur;
//       int val;
//       uintptr_t cardAddr;
//       const char* value;
//       const char* space;
//       const char* majorString = major ? "in major collection" : "in minor collection";
//       fprintf(f, "start of table %s \n", majorString);
//      for (int i = 0; i < _cur_covered_regions; i++) {
//          fprintf(f,"printing region %d: \n", i);
//        MemRegion mri = _whole_heap.intersection(_covered[i]);
//        if (!mri.is_empty()) {
//          jbyte *cur_entry, *next_entry, *limit;
//          for (cur_entry = byte_for(mri.start()), limit = byte_for(mri.last());
//               cur_entry <= limit;
...
//            next_entry = cur_entry + 1;
//            
//            cardAddr = (uintptr_t)(addr_for(cur_entry));
//            ParallelScavengeHeap* heap = (ParallelScavengeHeap*)Universe::heap();
////            assert(heap->kind() == CollectedHeap::ParallelScavengeHeap, "Sanity");
//            if(heap->young_gen()->eden_space()->contains(addr_for(cur_entry))) space = "in eden";
//            if(heap->young_gen()->from_space()->contains(addr_for(cur_entry))) space = "in from";
//            if(heap->young_gen()->to_space()->contains(addr_for(cur_entry))) space = "in to";
//            if(heap->old_gen()->is_in(addr_for(cur_entry))) space = "in old";
//            switch(*cur_entry) {
//                case clean_card: 
//                    value = "clean";
//                    break;
...
//            val = (int)(*cur_entry);
//            
//            fprintf(f, " Addr (%s): %lu, val: %d , %s \n", space, cardAddr, val,value);           
//          }
//        }
//      }
//       fprintf(f,"end of table \n\n");
//       fclose(f);       
//  }
