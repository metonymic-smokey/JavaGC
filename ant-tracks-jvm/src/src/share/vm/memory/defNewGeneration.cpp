    ...
    _should_allocate_from_space(false)
{
  ...
  if (TraceObjectsGC) {
    EventsRuntime::fire_space_creation(GENERATIONAL_GC_GEN_0_EDEN_ID,       eden()->bottom(), eden()->end());
    EventsRuntime::fire_space_creation(GENERATIONAL_GC_GEN_0_SURVIVOR_1_ID, from()->bottom(), from()->end());
    EventsRuntime::fire_space_creation(GENERATIONAL_GC_GEN_0_SURVIVOR_2_ID,   to()->bottom(),   to()->end());
...
bool DefNewGeneration::expand(size_t bytes) {
  ...
  if(TraceObjects) {
      // do nothing, only the virtual space is expanded here, we do not really care unless spaces are redefined
  }
...
void DefNewGeneration::compute_new_size() {
  ...
  if (changed) {
    ...
    if (TraceObjectsGC) {
        EventsGCRuntime::schedule_space_redefine(GENERATIONAL_GC_GEN_0_EDEN_ID,       eden()->bottom(), eden()->end());
        EventsGCRuntime::schedule_space_redefine(GENERATIONAL_GC_GEN_0_SURVIVOR_1_ID, from()->bottom(), from()->end());
        EventsGCRuntime::schedule_space_redefine(GENERATIONAL_GC_GEN_0_SURVIVOR_2_ID,   to()->bottom(),   to()->end());
    }
  }
}
...
void DefNewGeneration::collect(bool   full,
  ...
  uint survivor_id;
  if(TraceObjectsGC) {
      gc_id = EventsGCRuntime::fire_gc_start(GCType_Minor, gch->gc_cause(), false, false, _next_gen->kind() == Generation::MarkSweepCompact);
      HeapWord* fromBottom = from()->bottom();
      HeapWord* toBottom = to()->bottom();
      if(fromBottom < toBottom) survivor_id = GENERATIONAL_GC_GEN_0_SURVIVOR_1_ID;
      else if (toBottom < fromBottom) survivor_id = GENERATIONAL_GC_GEN_0_SURVIVOR_2_ID;
      else assert(false, "from and to survivor cannot be at same address!");
      EventsGCRuntime::fire_gc_info(GENERATIONAL_GC_GEN_0_EDEN_ID, gc_id);
      EventsGCRuntime::fire_gc_info(survivor_id, gc_id);
      EventsRuntime::fire_sync(false);
      EventSynchronization::do_synchronization_tasks_for_generation_n(1);
      //these PLAB events are just an optimization to avoid many small labs in parser due to move slows all generating individual labs
      EventsGCRuntime::fire_plab_alloc(_to_space->top(), 0);
      if(_next_gen->kind() == Generation::MarkSweepCompact) {
         EventsGCRuntime::fire_plab_alloc((((OneContigSpaceCardGeneration*) _next_gen))->used_region().end(), 0);
      }
  }
  ...
  if(TraceObjectsGC) {
      if(_promotion_failed) {
        ...
        EventsGCRuntime::fire_gc_failed(GENERATIONAL_GC_GEN_0_EDEN_ID);
        EventsGCRuntime::fire_gc_failed(survivor_id);
      }
      EventsGCRuntime::fire_gc_end(GCType_Minor, gc_id, gch->gc_cause(), _promotion_failed);
  }
}
...
void DefNewGeneration::handle_promotion_failure(oop old) {
  ...
  // forward to self
  old->forward_to(old);
  if(TraceObjectsGC) EventsGCRuntime::fire_gc_keep_alive(old);
...
oop DefNewGeneration::copy_to_survivor_space(oop old) {
  ...
  if (obj == NULL) {
    ...
    if (TraceObjectsGC) {
      assert(from()->is_in(old) || eden()->is_in(old), "not in from- or eden-space!");
      assert(to()->is_in(obj), "not in to-space!");
      EventsGCRuntime::fire_gc_move_slow(old, obj, (int) s);
    }
  }
