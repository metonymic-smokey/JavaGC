...
void InstanceKlass::release_C_heap_structures() {
  ...
  if(TraceObjectsAllocations) {
    AllocatedTypes::remove_allocated_type(this);
    //Not needed anymore, since we need it if same class is loaded again!
    //A class is only unloaded if the corresponding classloader is unloaded.
    //methods_do(AllocationSites::remove_allocation_site);
  }
