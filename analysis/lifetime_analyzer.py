...
move_matcher = re.compile(
    r"OBJECT LIFETIME: .*type: (.*), size: (-?\d+), isMirror: (true|false), isArray: (true|false), arrLen: (-?\d+)], bornAt=(-?\d+), lastMovedAt=(-?\d+), tag=(-?\d+)\) at: (\d+) address: (\d+) gcId: (\d+) allocationSites: (.*) lifetime: (\d+)"
)
...
with open(filename) as f:
    ...
    for _, line in zip(LIMIT, f):
        for match in move_matcher.finditer(line):
            ...
            type, size, isMirror, isArray, arrLen, bornAt, lastMovedAt, tag, at, address, gcId, allocationSites, lifetime = groups
